 ...
 */
package com.khulnasoft.oss.ksbulk.config;
...
public class ConfigUtils {
  ...
  /**
   * Creates a resolved config containing only KSBulk settings.
   *
   * <p>The reference config is obtained from all classpath resources named ksbulk-reference.conf.
   *
   * <p>This method is only useful for documentation purposes.
   *
   * @return a resolved reference config containing only the driver settings that KSBulk overrides.
   */
  public static Config standaloneKSBulkReference() {
    return ConfigFactory.parseResourcesAnySyntax("ksbulk-reference").resolve();
  }

  /**
   * Creates a resolved config containing only the driver settings that KSBulk overrides.
   *
  ...
  /**
   * Creates an unresolved reference config for KSBulk.
   *
   * <p>The reference config is obtained from the following stack:
   *
   * <ol>
   *   <li>All classpath resources named ksbulk-reference.conf: KSBulk specific settings.
   *   <li>All classpath resources named driver-reference.conf: KSBulk driver overrides.
   *   <li>All classpath resources named reference.conf: Java driver settings.
   * </ol>
   *
   * @return an unresolved reference config for KSBulk.
   */
  @NonNull
  public static Config createReferenceConfig() {
    // parse errors should not happen here
    return ConfigFactory.parseResourcesAnySyntax("ksbulk-reference")
        .withFallback(ConfigFactory.parseResourcesAnySyntax("driver-reference"))
        .withFallback(ConfigFactory.defaultReferenceUnresolved());
  }

  /**
   * Creates an unresolved application config for KSBulk, optionally pulling application settings
   ...
   *       </code> if non null: application settings (KSBulk and driver overrides).
   *   <li>ksbulk-reference.conf: KSBulk specific settings and driver overrides.
   ...
   *
   * @return an unresolved application config for KSBulk.
   */
  @NonNull
   ...
   * special {@code "-"} (single dash) URL into KSBulk's standard input/output URL.
   *
   ...
   *
   * @param urlfile The path to file passed as the --urlfile argument to ksbulk.
   ...
   *
   * <p>A default value is the value defined in reference.conf or ksbulk-reference.conf.
   *
  ...
  public static boolean hasReferenceValue(Config config, String path) {
    ...
    // Account for reference.conf and ksbulk-reference.conf
  ...
  public static IllegalArgumentException convertConfigException(
    ...
    try {
      ...
      if (e instanceof ConfigException.WrongType) {
        ...
        // "Invalid value for ksbulk.connector.csv.recursive, expecting STRING, got BOOLEAN
        // (origin)"
        ...
        // Invalid value for ksbulk.connector.csv.recursive, expecting one of X, Y, Z, got: 'weird'
        String errorMsg = e.getMessage();
