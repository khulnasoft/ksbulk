 ...
 * </ol>
 *
 * Connectors are allowed to be stateful. KSBulk will always preserve the order of method
 ...
 * implement {@link #read()} by emitting their records grouped by such resources; this allows KSBulk
 ...
 * preserved.
 *
 * <p>KSBulk will optimize reads according to the read concurrency. Implementors should implement
 ...
 *
 * <p>KSBulk will optimize writes according to the write concurrency. Implementors should implement
 ...
 * onError} signals to downstream subscribers. KSBulk will abort the operation immediately.
 *
 ...
 * This way, KSBulk is able to detect such errors and treat them accordingly (for example, by
 * redirecting them to a "bad file" or by updating a counter of errors).
 */
public interface Connector extends AutoCloseable {

  /**
   * Reads records from all the resources in the datasource.
   *
   * <p>Resources are guaranteed to be consumed with a parallelism no greater than the {@linkplain
   * #readConcurrency() read concurrency}.
   *
   * <p>If the underlying datasource cannot split its records into multiple resources in any
   * efficient manner, then this method should return a publisher of one single resource.
   *
   ...
   *
   * @return a {@link Publisher} of resources from the datasource.
   */
  @NonNull
  Publisher<Resource> read();
