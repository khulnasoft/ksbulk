 ...
 */
package com.khulnasoft.oss.ksbulk.workflow.commons.ssl;

import static com.khulnasoft.oss.ksbulk.io.IOUtils.assertAccessibleFile;

import com.khulnasoft.oss.driver.api.core.ssl.ProgrammaticSslEngineFactory;
import com.khulnasoft.oss.driver.api.core.ssl.SslEngineFactory;
...
public class SslHandlerFactoryFactory {
    ...
    switch (sslProvider.toLowerCase()) {
      ...
      default:
        throw new IllegalArgumentException(
            String.format(
                "Invalid value for ksbulk.driver.ssl.provider, expecting None, JDK, or OpenSSL, got: '%s'",
                sslProvider));
    }
  }
  ...
  private static SslHandlerFactory createNettySslHandlerFactory(Config config)
    ...
    if (config.hasPath("openssl.keyCertChain") != config.hasPath("openssl.privateKey")) {
      throw new IllegalArgumentException(
          "Settings "
              + "ksbulk.driver.ssl.openssl.keyCertChain"
              + " and "
              + "ksbulk.driver.ssl.openssl.privateKey"
  ...
  private static TrustManagerFactory createTrustManagerFactory(Config config)
    ...
    if (config.hasPath("truststore.path") != config.hasPath("truststore.password")) {
      throw new IllegalArgumentException(
          "Settings "
              + "ksbulk.driver.ssl.truststore.path"
              + ", "
              + "ksbulk.driver.ssl.truststore.password"
              + " and "
              + "ksbulk.driver.ssl.truststore.algorithm"
  ...
  private static KeyManagerFactory createKeyManagerFactory(Config config)
    ...
    if (config.hasPath("keystore.path") != config.hasPath("keystore.password")) {
      throw new IllegalArgumentException(
          "Settings "
              + "ksbulk.driver.ssl.keystore.path"
              + ", "
              + "ksbulk.driver.ssl.keystore.password"
              + " and "
              + "ksbulk.driver.ssl.keystore.algorithm"
