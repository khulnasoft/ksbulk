 ...
 */
package com.khulnasoft.oss.ksbulk.workflow.commons.metrics.prometheus;
...
import com.khulnasoft.oss.ksbulk.workflow.commons.metrics.prometheus.PrometheusManager.PullConfig;
import com.khulnasoft.oss.ksbulk.workflow.commons.metrics.prometheus.PrometheusManager.PushConfig;
    ...
    try {
      ...
      assertThat(data)
          .anySatisfy(
              line ->
                  assertThat(line)
                      .startsWith("ksbulk_records_total")
                      .contains("application_version=")
                      .contains("client_id=")
                      .contains("driver_version=")
                      .contains("application_name=\"KhulnaSoft Bulk Loader execution1\"")
  ...
  void should_push_metrics_to_gateway(
    ...
    String expectedLabelsAsString =
        "{name1=\"value1\",application_version=\""
            + version
            + "\",application_name=\"KhulnaSoft Bulk Loader execution1\",client_id=\"de13b396-eb09-31d1-9876-cba56b790be0\",driver_version=\""
            + driverVersion
    ...
    RequestPatternBuilder builder =
        postRequestedFor(urlPathMatching(pathRegex))
            ...
            .withRequestBody(
                containing("ksbulk_elapsed_time_seconds" + expectedLabelsAsString + " 123.0"))
            .withRequestBody(
                containing("ksbulk_records_total" + expectedLabelsAsString + " 123456.0"));

    if (success) {
      builder =
          builder
              .withRequestBody(containing("ksbulk_success" + expectedLabelsAsString + " 1.0"))
              .withRequestBody(containing("ksbulk_last_success" + expectedLabelsAsString));
    } else {
      builder =
          builder
              .withRequestBody(containing("ksbulk_success" + expectedLabelsAsString + " 0.0"))
              .withRequestBody(notMatching(".*ksbulk_last_success.*"));
    }
