...
import static com.khulnasoft.oss.driver.api.core.DefaultProtocolVersion.V3;
import static com.khulnasoft.oss.driver.api.core.DefaultProtocolVersion.V4;
import static com.khulnasoft.oss.driver.api.core.type.DataTypes.BIGINT;
import static com.khulnasoft.oss.driver.api.core.type.DataTypes.COUNTER;
...
import static com.khulnasoft.oss.ksbulk.tests.driver.DriverUtils.mockColumnDefinition;
import static com.khulnasoft.oss.ksbulk.tests.driver.DriverUtils.mockColumnDefinitions;
import static com.khulnasoft.oss.ksbulk.tests.driver.DriverUtils.mockSession;
import static com.khulnasoft.oss.ksbulk.tests.driver.DriverUtils.newToken;
import static com.khulnasoft.oss.ksbulk.tests.driver.DriverUtils.newTokenRange;
...
import static com.khulnasoft.oss.ksbulk.workflow.commons.schema.QueryInspector.INTERNAL_TIMESTAMP_VARNAME;
import static com.khulnasoft.oss.ksbulk.workflow.commons.settings.SchemaGenerationStrategy.MAP_AND_WRITE;
import static com.khulnasoft.oss.ksbulk.workflow.commons.settings.SchemaGenerationStrategy.READ_AND_COUNT;
import static com.khulnasoft.oss.ksbulk.workflow.commons.settings.SchemaGenerationStrategy.READ_AND_MAP;
import static com.khulnasoft.oss.ksbulk.workflow.commons.settings.StatsSettings.StatisticsMode.global;
import static com.khulnasoft.oss.ksbulk.workflow.commons.settings.StatsSettings.StatisticsMode.hosts;
import static com.khulnasoft.oss.ksbulk.workflow.commons.settings.StatsSettings.StatisticsMode.partitions;
import static com.khulnasoft.oss.ksbulk.workflow.commons.settings.StatsSettings.StatisticsMode.ranges;
...
import com.khulnasoft.dse.driver.api.core.metadata.schema.DseEdgeMetadata;
import com.khulnasoft.dse.driver.api.core.metadata.schema.DseGraphKeyspaceMetadata;
import com.khulnasoft.dse.driver.api.core.metadata.schema.DseGraphTableMetadata;
import com.khulnasoft.dse.driver.api.core.metadata.schema.DseVertexMetadata;
...
import com.khulnasoft.oss.driver.api.core.metadata.schema.IndexMetadata;
import com.khulnasoft.oss.driver.api.core.metadata.schema.ViewMetadata;
import com.khulnasoft.oss.driver.api.core.metadata.token.Token;
import com.khulnasoft.oss.driver.api.core.metadata.token.TokenRange;
import com.khulnasoft.oss.driver.api.core.servererrors.SyntaxError;
...
import com.khulnasoft.oss.driver.internal.core.metadata.token.DefaultTokenMap;
...
import com.khulnasoft.oss.driver.shaded.guava.common.collect.ImmutableSetMultimap;
import com.khulnasoft.oss.driver.shaded.guava.common.collect.Lists;
import com.khulnasoft.oss.driver.shaded.guava.common.collect.SetMultimap;
import com.khulnasoft.oss.driver.shaded.guava.common.collect.Sets;
import com.khulnasoft.oss.ksbulk.codecs.api.CommonConversionContext;
...
import com.khulnasoft.oss.ksbulk.codecs.api.format.temporal.CqlTemporalFormat;
import com.khulnasoft.oss.ksbulk.codecs.api.format.temporal.TemporalFormat;
import com.khulnasoft.oss.ksbulk.codecs.text.string.StringToInstantCodec;
import com.khulnasoft.oss.ksbulk.connectors.api.DefaultIndexedField;
...
import com.khulnasoft.oss.ksbulk.mapping.DefaultMapping;
...
import com.khulnasoft.oss.ksbulk.workflow.commons.schema.NestedBatchException;
import com.khulnasoft.oss.ksbulk.workflow.commons.schema.QueryInspector;
import com.khulnasoft.oss.ksbulk.workflow.commons.schema.ReadResultCounter;
import com.khulnasoft.oss.ksbulk.workflow.commons.schema.ReadResultMapper;
import com.khulnasoft.oss.ksbulk.workflow.commons.schema.RecordMapper;
import com.khulnasoft.oss.ksbulk.workflow.commons.statement.RangeReadBoundStatement;
...
class SchemaSettingsTest {
  ...
  void should_create_record_mapper_when_mapping_keyspace_and_counter_table_provided() {
    ...
    Config config =
        TestConfigUtils.createTestConfig("ksbulk.schema", "keyspace", "ks", "table", "t1");
    SchemaSettings settings = new SchemaSettings(config, MAP_AND_WRITE);
  ...
  void should_error_out_when_counter_table_and_mapping_has_function_calls() {
    ...
    Config config =
        TestConfigUtils.createTestConfig(
            "ksbulk.schema", "keyspace", "ks", "table", "t1", "mapping", "\"f1=c1,now()=c3\"");
    SchemaSettings settings = new SchemaSettings(config, MAP_AND_WRITE);
    settings.init(session, codecFactory, true, true);
    assertThatThrownBy(() -> settings.createRecordMapper(session, recordMetadata, false))
        .isInstanceOf(IllegalArgumentException.class)
  ...
  void should_create_record_mapper_when_mapping_is_a_list_and_indexed() {
    Config config =
        TestConfigUtils.createTestConfig(
            "ksbulk.schema", "mapping", "\"\\\"COL 2\\\", c1\", ", "keyspace", "ks", "table", "t1");
    SchemaSettings settings = new SchemaSettings(config, MAP_AND_WRITE);
    settings.init(session, codecFactory, true, false);
    RecordMapper mapper = settings.createRecordMapper(session, recordMetadata, false);
    assertThat(mapper).isNotNull();
  ...
  void should_create_record_mapper_with_inferred_mapping_and_override() {
    ...
    Config config =
        TestConfigUtils.createTestConfig(
            "ksbulk.schema", "keyspace", "ks", "table", "t1", "mapping", "\" *=*, c4 = c3 \"");
    SchemaSettings settings = new SchemaSettings(config, MAP_AND_WRITE);
    settings.init(session, codecFactory, false, true);
    RecordMapper mapper = settings.createRecordMapper(session, recordMetadata, false);
    assertThat(mapper).isNotNull();
  ...
  void should_create_record_mapper_with_inferred_mapping_and_skip() {
    ...
    Config config =
        TestConfigUtils.createTestConfig(
            "ksbulk.schema", "keyspace", "ks", "table", "t1", "mapping", "\" *=-\\\"COL 2\\\" \"");
    SchemaSettings settings = new SchemaSettings(config, MAP_AND_WRITE);
    settings.init(session, codecFactory, false, true);
    RecordMapper mapper = settings.createRecordMapper(session, recordMetadata, false);
    assertThat(mapper).isNotNull();
  ...
  void should_create_row_mapper_when_mapping_keyspace_and_table_provided() {
    ...
    settings.init(session, codecFactory, true, false);
    ReadResultMapper mapper = settings.createReadResultMapper(session, recordMetadata, true);
    assertThat(mapper).isNotNull();
    ArgumentCaptor<String> argument = ArgumentCaptor.forClass(String.class);
    verify(session).prepare(argument.capture());
    assertThat(argument.getValue())
  ...
  void should_include_function_call_in_select_statement() {
    ...
    SchemaSettings settings = new SchemaSettings(config, READ_AND_MAP);
    settings.init(session, codecFactory, false, true);
    settings.createReadResultMapper(session, recordMetadata, true);
    assertThat(getInternalState(settings, "query"))
        .isEqualTo(
  ...
  void should_create_single_read_statement_when_no_variables() {
    ...
    when(bs.setRoutingKeyspace(any(CqlIdentifier.class))).thenReturn(bs);
    when(bs.setRoutingToken(any(Token.class))).thenReturn(bs);
    when(ps.bind()).thenReturn(bs);
    Config config =
        TestConfigUtils.createTestConfig("ksbulk.schema", "query", "\"SELECT a,b,c FROM ks.t1\"");
    SchemaSettings settings = new SchemaSettings(config, READ_AND_MAP);
    settings.init(session, codecFactory, false, true);
    settings.createReadResultMapper(session, recordMetadata, true);
    List<?> statements = settings.createReadStatements(session);
    assertThat(statements).hasSize(1);
    assertThat(statements).extracting("delegate").containsOnly(bs);
  }
  ...
  void should_create_multiple_read_statements() {
    ...
    Config config =
        TestConfigUtils.createTestConfig(
            "ksbulk.schema", "keyspace", "ks", "table", "t1", "splits", 3);
    SchemaSettings settings = new SchemaSettings(config, READ_AND_MAP);
    settings.init(session, codecFactory, false, true);
    settings.createReadResultMapper(session, recordMetadata, true);
    List<RangeReadBoundStatement> statements = settings.createReadStatements(session);
    assertThat(statements).extracting("delegate").hasSize(3).contains(bs1, bs2, bs3);
  }
  ...
  void should_use_custom_query_when_mode_is_global() {
    ...
    Config config =
        TestConfigUtils.createTestConfig(
            "ksbulk.schema", "query", "\"SELECT c1, c3 FROM ks.t1 WHERE c1 = 0\"");
    SchemaSettings settings = new SchemaSettings(config, READ_AND_COUNT);
    settings.init(session, codecFactory, false, true);
    ReadResultCounter counter =
        settings.createReadResultCounter(session, codecFactory, EnumSet.of(global), 10);
  ...
  void should_warn_that_table_was_not_found_but_similar_mv_exists() {
    Config config =
        TestConfigUtils.createTestConfig("ksbulk.schema", "keyspace", "ks", "table", "MV1");
    SchemaSettings settings = new SchemaSettings(config, READ_AND_MAP);
    assertThatThrownBy(() -> settings.init(session, codecFactory, false, true))
        .isInstanceOf(IllegalArgumentException.class)
        .hasMessage(
  ...
  void should_warn_that_keyspace_was_not_found() {
    Config config =
        TestConfigUtils.createTestConfig("ksbulk.schema", "keyspace", "MyKs", "table", "t1");
    SchemaSettings settings = new SchemaSettings(config, MAP_AND_WRITE);
    assertThatThrownBy(() -> settings.init(session, codecFactory, false, true))
        .isInstanceOf(IllegalArgumentException.class)
  ...
  void should_warn_that_table_was_not_found() {
    Config config =
        TestConfigUtils.createTestConfig("ksbulk.schema", "keyspace", "ks", "table", "MyTable");
    SchemaSettings settings = new SchemaSettings(config, MAP_AND_WRITE);
    assertThatThrownBy(() -> settings.init(session, codecFactory, false, true))
        .isInstanceOf(IllegalArgumentException.class)
  ...
  void should_warn_that_mapped_fields_not_supported() {
    Config config =
        TestConfigUtils.createTestConfig(
            "ksbulk.schema", "keyspace", "ks", "table", "t1", "mapping", "\"c1=c1\"");
    SchemaSettings settings = new SchemaSettings(config, MAP_AND_WRITE);
  ...
  void should_error_invalid_schema_settings() {
    Config config = TestConfigUtils.createTestConfig("ksbulk.schema");
    SchemaSettings settings = new SchemaSettings(config, MAP_AND_WRITE);
    assertThatThrownBy(() -> settings.init(session, codecFactory, true, false))
        .isInstanceOf(IllegalArgumentException.class)
        .hasMessageContaining(
  ...
  void should_error_invalid_schema_mapping_missing_keyspace_and_table() {
    Config config = TestConfigUtils.createTestConfig("ksbulk.schema", "mapping", "\"c1=c2\"");
    SchemaSettings settings = new SchemaSettings(config, MAP_AND_WRITE);
    assertThatThrownBy(() -> settings.init(session, codecFactory, true, false))
        .isInstanceOf(IllegalArgumentException.class)
        .hasMessageContaining(
  ...
  void should_error_when_query_is_qualified_and_keyspace_provided() {
    Config config =
        TestConfigUtils.createTestConfig(
            "ksbulk.schema", "query", "\"INSERT INTO ks.t1 (col1) VALUES (?)\"", "keyspace", "ks");
    SchemaSettings settings = new SchemaSettings(config, MAP_AND_WRITE);
    assertThatThrownBy(() -> settings.init(session, codecFactory, true, false))
        .isInstanceOf(IllegalArgumentException.class)
        .hasMessageContaining(
  ...
  void should_error_when_query_is_not_qualified_and_keyspace_not_provided() {
    Config config =
        TestConfigUtils.createTestConfig(
            "ksbulk.schema", "query", "\"INSERT INTO t1 (col1) VALUES (?)\"");
    SchemaSettings settings = new SchemaSettings(config, MAP_AND_WRITE);
    assertThatThrownBy(() -> settings.init(session, codecFactory, true, false))
        .isInstanceOf(IllegalArgumentException.class)
        .hasMessageContaining(
  ...
  void should_error_when_query_is_qualified_and_keyspace_non_existent() {
    ...
    Config config =
        TestConfigUtils.createTestConfig(
            "ksbulk.schema", "query", "\"INSERT INTO ks.t1 (col1) VALUES (?)\"");
    SchemaSettings settings = new SchemaSettings(config, MAP_AND_WRITE);
    assertThatThrownBy(() -> settings.init(session, codecFactory, true, false))
        .isInstanceOf(IllegalArgumentException.class)
  ...
  void should_error_when_mapping_provided_and_count_workflow() {
    Config config =
        TestConfigUtils.createTestConfig(
            "ksbulk.schema", "keyspace", "ks", "table", "t1", "mapping", "\"col1,col2\"");
    SchemaSettings settings = new SchemaSettings(config, READ_AND_COUNT);
    assertThatThrownBy(() -> settings.init(session, codecFactory, true, false))
        .isInstanceOf(IllegalArgumentException.class)
  ...
  void should_error_invalid_timestamp() {
    Config config =
        TestConfigUtils.createTestConfig(
            "ksbulk.schema", "queryTimestamp", "junk", "keyspace", "ks", "table", "t1");
    SchemaSettings settings = new SchemaSettings(config, MAP_AND_WRITE);
    assertThatThrownBy(() -> settings.init(session, codecFactory, true, false))
        .isInstanceOf(IllegalArgumentException.class)
        .hasMessageContaining(
  ...
  void should_error_invalid_schema_missing_keyspace() {
    Config config = TestConfigUtils.createTestConfig("ksbulk.schema", "table", "t1");
    SchemaSettings settings = new SchemaSettings(config, MAP_AND_WRITE);
    assertThatThrownBy(() -> settings.init(session, codecFactory, true, false))
        .isInstanceOf(IllegalArgumentException.class)
        .hasMessageContaining(
  ...
  void should_throw_exception_when_nullToUnset_not_a_boolean() {
    Config config =
        TestConfigUtils.createTestConfig(
            "ksbulk.schema", "keyspace", "ks", "table", "t1", "nullToUnset", "NotABoolean");
    SchemaSettings settings = new SchemaSettings(config, MAP_AND_WRITE);
    assertThatThrownBy(() -> settings.init(session, codecFactory, true, false))
        .isInstanceOf(IllegalArgumentException.class)
        .hasMessageContaining(
            "Invalid value for ksbulk.schema.nullToUnset, expecting BOOLEAN, got STRING");
  }
  ...
  void should_error_when_graph_options_provided_but_cluster_is_not_compatible() {
    ...
    Config config =
        TestConfigUtils.createTestConfig("ksbulk.schema", "graph", "ks", "vertex", "v1");
    SchemaSettings settings = new SchemaSettings(config, MAP_AND_WRITE);
    assertThatThrownBy(() -> settings.init(session, codecFactory, false, true))
  ...
  void should_error_when_mapping_does_not_contain_primary_key() {
    Config config =
        TestConfigUtils.createTestConfig(
            "ksbulk.schema", "keyspace", "ks", "table", "t1", "mapping", "\"fieldA = c3\"");
    SchemaSettings settings = new SchemaSettings(config, MAP_AND_WRITE);
    settings.init(session, codecFactory, false, true);
    assertThatThrownBy(() -> settings.createRecordMapper(session, recordMetadata, false))
        .isInstanceOf(IllegalArgumentException.class)
  ...
  void should_error_when_insert_query_does_not_contain_primary_key() {
    ...
    Config config =
        TestConfigUtils.createTestConfig(
            "ksbulk.schema", "query", "\"INSERT INTO ks.t1 (c2) VALUES (:c2)\"");
    SchemaSettings settings = new SchemaSettings(config, MAP_AND_WRITE);
    settings.init(session, codecFactory, false, true);
    assertThatThrownBy(() -> settings.createRecordMapper(session, recordMetadata, false))
        .isInstanceOf(IllegalArgumentException.class)
  ...
  void
    ...
    Config config =
        TestConfigUtils.createTestConfig(
            "ksbulk.schema", "query", "\"INSERT INTO ks.t1 (c1, c3) VALUES (:c1, :c3)\"");
    when(table.getPrimaryKey()).thenReturn(newArrayList(col1, col2));
  ...
  void should_insert_where_clause_in_select_statement_simple() {
    ...
    Config config =
        TestConfigUtils.createTestConfig(
            "ksbulk.schema", "keyspace", "ks", "query", "\"SELECT a,b,c FROM t1\"", "splits", 3);
    SchemaSettings settings = new SchemaSettings(config, READ_AND_MAP);
    settings.init(session, codecFactory, false, true);
    settings.createReadResultMapper(session, recordMetadata, true);
  ...
  void should_error_when_graph_and_keyspace_both_present() {
    Config config =
        TestConfigUtils.createTestConfig("ksbulk.schema", "keyspace", "ks", "graph", "graph1");
    SchemaSettings settings = new SchemaSettings(config, MAP_AND_WRITE);
    assertThatThrownBy(() -> settings.init(session, codecFactory, true, false))
        .isInstanceOf(IllegalArgumentException.class)
  ...
  void should_error_when_table_and_vertex_both_present() {
    Config config =
        TestConfigUtils.createTestConfig("ksbulk.schema", "table", "t1", "vertex", "v1");
    SchemaSettings settings = new SchemaSettings(config, MAP_AND_WRITE);
    assertThatThrownBy(() -> settings.init(session, codecFactory, true, false))
        .isInstanceOf(IllegalArgumentException.class)
  ...
  void should_error_when_table_and_edge_both_present() {
    Config config = TestConfigUtils.createTestConfig("ksbulk.schema", "table", "t1", "edge", "e1");
    SchemaSettings settings = new SchemaSettings(config, MAP_AND_WRITE);
    assertThatThrownBy(() -> settings.init(session, codecFactory, true, false))
        .isInstanceOf(IllegalArgumentException.class)
  ...
  void should_error_when_vertex_and_edge_both_present() {
    Config config = TestConfigUtils.createTestConfig("ksbulk.schema", "vertex", "v1", "edge", "e1");
    SchemaSettings settings = new SchemaSettings(config, MAP_AND_WRITE);
    assertThatThrownBy(() -> settings.init(session, codecFactory, true, false))
        .isInstanceOf(IllegalArgumentException.class)
  ...
  void should_error_when_edge_without_from_vertex() {
    Config config = TestConfigUtils.createTestConfig("ksbulk.schema", "edge", "e1");
    SchemaSettings settings = new SchemaSettings(config, MAP_AND_WRITE);
    assertThatThrownBy(() -> settings.init(session, codecFactory, true, false))
        .isInstanceOf(IllegalArgumentException.class)
  ...
  void should_error_when_edge_without_to_vertex() {
    Config config = TestConfigUtils.createTestConfig("ksbulk.schema", "edge", "e1", "from", "v1");
    SchemaSettings settings = new SchemaSettings(config, MAP_AND_WRITE);
    assertThatThrownBy(() -> settings.init(session, codecFactory, true, false))
        .isInstanceOf(IllegalArgumentException.class)
  ...
  void should_not_insert_where_clause_in_select_statement_if_already_exists() {
    ...
    BoundStatement bs1 = mock(BoundStatement.class);
    when(bs1.setRoutingKeyspace(any(CqlIdentifier.class))).thenReturn(bs1);
    when(bs1.setRoutingToken(any(Token.class))).thenReturn(bs1);
    when(ps.bind()).thenReturn(bs1);
    Config config =
        TestConfigUtils.createTestConfig(
            "ksbulk.schema", "keyspace", "ks", "query", "\"SELECT a,b,c FROM t1 WHERE c1 = 1\"");
    SchemaSettings settings = new SchemaSettings(config, READ_AND_MAP);
    settings.init(session, codecFactory, false, true);
    settings.createReadResultMapper(session, recordMetadata, true);
    List<?> stmts = settings.createReadStatements(session);
  ...
  void should_disable_null_to_unset_when_protocol_version_lesser_than_4(ProtocolVersion version) {
    ...
    Config config =
        TestConfigUtils.createTestConfig(
            "ksbulk.schema", "nullToUnset", true, "keyspace", "ks", "table", "t1");
    SchemaSettings settings = new SchemaSettings(config, MAP_AND_WRITE);
    settings.init(session, codecFactory, true, false);
    RecordMapper mapper = settings.createRecordMapper(session, recordMetadata, false);
  ...
  void should_infer_insert_query_without_solr_query_column() {
    ...
    when(idx.getClassName())
        .thenReturn(Optional.of("com.khulnasoft.bdp.search.solr.Cql3SolrSecondaryIndex"));
    Config config =
        TestConfigUtils.createTestConfig("ksbulk.schema", "keyspace", "ks", "table", "t1");
    SchemaSettings settings = new SchemaSettings(config, MAP_AND_WRITE);
    settings.init(session, codecFactory, false, true);
  ...
  void should_error_when_edge_non_existent() {
    Config config =
        TestConfigUtils.createTestConfig(
            "ksbulk.schema", "graph", "ks", "edge", "e1", "from", "v1", "to", "v2");
    SchemaSettings settings = new SchemaSettings(config, MAP_AND_WRITE);
    assertThatThrownBy(() -> settings.init(session, codecFactory, true, false))
        .isInstanceOf(IllegalArgumentException.class)
  ...
  void should_generate_single_read_statement_when_query_not_parallelizable() {
    Config config =
        TestConfigUtils.createTestConfig(
            "ksbulk.schema", "query", "\"select * from ks.t1 LIMIT 10\"");
    SchemaSettings settings = new SchemaSettings(config, MAP_AND_WRITE);
    settings.init(session, codecFactory, true, true);
  ...
  void should_infer_batch_insert_query(String mapping, String expectedQuery) {
    Config config =
        TestConfigUtils.createTestConfig(
            "ksbulk.schema", "keyspace", "ks", "table", "t1", "mapping", mapping);
    SchemaSettings settings = new SchemaSettings(config, MAP_AND_WRITE);
    settings.init(session, codecFactory, true, true);
    settings.createRecordMapper(session, recordMetadata, false);
  ...
  void should_infer_select_insert_query_with_timestamp_and_ttl_functions() {
    ...
    SchemaSettings settings = new SchemaSettings(config, READ_AND_MAP);
    settings.init(session, codecFactory, true, true);
    settings.createReadResultMapper(session, recordMetadata, false);
    assertThat(getInternalState(settings, "query"))
        .isEqualTo(
            "SELECT c1, "
  ...
  void should_not_preserve_timestamp_when_unsupported_type() {
    ...
    Config config =
        TestConfigUtils.createTestConfig(
            "ksbulk.schema", "keyspace", "ks", "table", "t1", "preserveTimestamp", true);
    SchemaSettings settings = new SchemaSettings(config, READ_AND_MAP);
    settings.init(session, codecFactory, true, true);
    settings.createReadResultMapper(session, recordMetadata, true);
    assertThat(getInternalState(settings, "query"))
  ...
  void should_not_preserve_ttl_when_unsupported_type() {
    ...
    Config config =
        TestConfigUtils.createTestConfig(
            "ksbulk.schema", "keyspace", "ks", "table", "t1", "preserveTtl", true);
    SchemaSettings settings = new SchemaSettings(config, READ_AND_MAP);
    settings.init(session, codecFactory, true, true);
    settings.createReadResultMapper(session, recordMetadata, true);
    assertThat(getInternalState(settings, "query"))
  ...
  void should_unwrap_batch_query(String query) {
    Config config =
        TestConfigUtils.createTestConfig("ksbulk.schema", "query", StringUtils.quoteJson(query));
    SchemaSettings settings = new SchemaSettings(config, MAP_AND_WRITE);
    settings.init(session, codecFactory, true, true);
    settings.createRecordMapper(session, recordMetadata, true);
  ...
  void should_reject_invalid_constant_in_mapping(
    ...
    Config config = TestConfigUtils.createTestConfig("ksbulk.schema", "mapping", mapping);
    if (query != null) {
