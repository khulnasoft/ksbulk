...
public class PrometheusManager {
  ...
  private void addFinalMetrics(
    ...
    Gauge.build()
        .name("ksbulk_elapsed_time_seconds")
        .help("Duration of KSBulk execution in seconds.")
    ...
    Gauge.build()
        .name("ksbulk_success")
        .help("Whether KSBulk execution completed successfully.")
        .labelNames(labelNames)
        .register(collectorRegistry)
        .labels(labelValues)
        .set(success ? 1 : 0);
    if (success) {
      Gauge.build()
          .name("ksbulk_last_success")
          .help("Last time KSBulk completed successfully, in unixtime.")
          .labelNames(labelNames)
          .register(collectorRegistry)
          .labels(labelValues)
          .setToCurrentTime();
      ...
      Gauge.build()
          .name("ksbulk_last_failure")
          .help("Last time KSBulk failed, in unixtime.")
          .labelNames(labelNames)
          .register(collectorRegistry)
          .labels(labelValues)
          .setToCurrentTime();
