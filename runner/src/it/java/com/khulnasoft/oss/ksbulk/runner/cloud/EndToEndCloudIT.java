 ...
 */
package com.khulnasoft.oss.ksbulk.runner.cloud;
...
import com.khulnasoft.oss.ksbulk.tests.cloud.SNIProxyServer;
import com.khulnasoft.oss.ksbulk.tests.cloud.SNIProxyServerExtension;
import com.khulnasoft.oss.ksbulk.tests.driver.annotations.SessionConfig;
import com.khulnasoft.oss.ksbulk.tests.driver.annotations.SessionConfig.UseKeyspaceMode;
...
class EndToEndCloudIT {
  ...
  private void performLoad(String... specificArgs) throws IOException, URISyntaxException {
    ...
    ExitStatus status = new KhulnaSoftBulkLoader(loadArgs.toArray(new String[0])).run();
  ...
  private void performUnload(String... specificArgs) throws IOException {
    ...
    ExitStatus status = new KhulnaSoftBulkLoader(unloadArgs.toArray(new String[0])).run();
    assertStatus(status, STATUS_OK);
  ...
  private List<String> commonArgs() {
    return Lists.newArrayList(
        ...
        "--khulnasoft-java-driver.advanced.connection.pool.local.size",
        "1",
        "--khulnasoft-java-driver.advanced.auth-provider.class",
