...
public class JsonNodeToVectorCodecTest {
  ...
  private final JsonNodeToVectorCodec ksbulkCodec =
  ...
  void should_encode_too_many_but_not_too_few() {
    ...
    JsonNode tooManyNode = ksbulkCodec.internalToExternal(tooManyVector);
    ArrayList<Float> tooFew = Lists.newArrayList(values);
    tooFew.remove(0);
    CqlVector<Float> tooFewVector = CqlVector.newInstance(tooFew);
    JsonNode tooFewNode = ksbulkCodec.internalToExternal(tooFewVector);

    assertThat(ksbulkCodec.encode(tooManyNode, ProtocolVersion.DEFAULT)).isNotNull();
    assertThatThrownBy(() -> ksbulkCodec.encode(tooFewNode, ProtocolVersion.DEFAULT))
