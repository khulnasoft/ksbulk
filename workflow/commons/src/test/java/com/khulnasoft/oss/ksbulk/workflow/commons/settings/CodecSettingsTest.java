...
import com.khulnasoft.oss.ksbulk.codecs.api.format.binary.Base64BinaryFormat;
import com.khulnasoft.oss.ksbulk.codecs.api.format.binary.HexBinaryFormat;
import com.khulnasoft.oss.ksbulk.codecs.text.json.JsonNodeToUnknownTypeCodec;
import com.khulnasoft.oss.ksbulk.codecs.text.string.StringToBigDecimalCodec;
import com.khulnasoft.oss.ksbulk.codecs.text.string.StringToBigIntegerCodec;
import com.khulnasoft.oss.ksbulk.codecs.text.string.StringToBooleanCodec;
import com.khulnasoft.oss.ksbulk.codecs.text.string.StringToByteCodec;
import com.khulnasoft.oss.ksbulk.codecs.text.string.StringToDoubleCodec;
import com.khulnasoft.oss.ksbulk.codecs.text.string.StringToFloatCodec;
import com.khulnasoft.oss.ksbulk.codecs.text.string.StringToInstantCodec;
import com.khulnasoft.oss.ksbulk.codecs.text.string.StringToIntegerCodec;
import com.khulnasoft.oss.ksbulk.codecs.text.string.StringToListCodec;
import com.khulnasoft.oss.ksbulk.codecs.text.string.StringToLocalDateCodec;
import com.khulnasoft.oss.ksbulk.codecs.text.string.StringToLocalTimeCodec;
import com.khulnasoft.oss.ksbulk.codecs.text.string.StringToLongCodec;
import com.khulnasoft.oss.ksbulk.codecs.text.string.StringToMapCodec;
import com.khulnasoft.oss.ksbulk.codecs.text.string.StringToSetCodec;
import com.khulnasoft.oss.ksbulk.codecs.text.string.StringToShortCodec;
import com.khulnasoft.oss.ksbulk.codecs.text.string.StringToTupleCodec;
import com.khulnasoft.oss.ksbulk.codecs.text.string.StringToUDTCodec;
import com.khulnasoft.oss.ksbulk.codecs.text.string.StringToUUIDCodec;
import com.khulnasoft.oss.ksbulk.codecs.text.string.StringToUnknownTypeCodec;
...
class CodecSettingsTest {
  ...
  void should_return_string_converting_codecs() {
    ...
    Config config = TestConfigUtils.createTestConfig("ksbulk.codec");
  ...
  void should_return_rounding_codecs() {
    Config config =
        TestConfigUtils.createTestConfig(
            "ksbulk.codec", "roundingStrategy", "UP", "formatNumbers", "true");
    CodecSettings settings = new CodecSettings(config);
    settings.init();
    ConvertingCodecFactory codecFactory = settings.createCodecFactory(false, false);
  ...
  void should_return_codecs_honoring_overflow_strategy() {
    Config config =
        TestConfigUtils.createTestConfig("ksbulk.codec", "overflowStrategy", "TRUNCATE");
    CodecSettings settings = new CodecSettings(config);
    settings.init();
    ConvertingCodecFactory codecFactory = settings.createCodecFactory(false, false);
  ...
  void should_create_settings_when_null_words_are_specified() {
    {
      Config config = TestConfigUtils.createTestConfig("ksbulk.codec", "nullStrings", "[NULL]");
    ...
    {
      Config config =
          TestConfigUtils.createTestConfig("ksbulk.codec", "nullStrings", "[NIL, NULL]");
      CodecSettings codecSettings = new CodecSettings(config);
      codecSettings.init();
      @SuppressWarnings("unchecked")
      List<String> nullStrings =
  ...
  void should_throw_exception_when_nullStrings_not_a_list() {
    ...
    assertThatThrownBy(settings::init)
        ...
        .hasMessageContaining(
            "Invalid value for ksbulk.codec.nullStrings, expecting LIST, got NULL");
  }

  @Test
  void should_throw_exception_when_epoch_not_in_ISO_ZONED_DATE_TIME() {
    Config config = TestConfigUtils.createTestConfig("ksbulk.codec", "epoch", "NotAValidTemporal");
    CodecSettings settings = new CodecSettings(config);
    assertThatThrownBy(settings::init)
        .isInstanceOf(IllegalArgumentException.class)
        .hasMessageContaining(
            "Invalid value for ksbulk.codec.epoch, expecting temporal in ISO_ZONED_DATE_TIME format, got 'NotAValidTemporal'");
  }

  @Test
  void should_throw_exception_when_booleanNumbers_not_a_list_of_two_elements() {
    Config config = TestConfigUtils.createTestConfig("ksbulk.codec", "booleanNumbers", "[0,1,2]");
    CodecSettings settings = new CodecSettings(config);
    assertThatThrownBy(settings::init)
        .isInstanceOf(IllegalArgumentException.class)
        .hasMessageContaining(
            "Invalid value for ksbulk.codec.booleanNumbers, expecting list with two numbers, got '[0, 1, 2]'");
  }

  @Test
  void should_throw_exception_when_booleanNumbers_not_a_list_of_numbers() {
    Config config =
        TestConfigUtils.createTestConfig("ksbulk.codec", "booleanNumbers", "[0,NotANumber]");
    CodecSettings settings = new CodecSettings(config);
    assertThatThrownBy(settings::init)
        .isInstanceOf(IllegalArgumentException.class)
        .hasMessageContaining(
            "Invalid value for ksbulk.codec.booleanNumbers, expecting list with two numbers, got '[0, NotANumber]'");
  }

  @Test
  void should_create_settings_when_valid_booleanNumbers() {
    Config config = TestConfigUtils.createTestConfig("ksbulk.codec", "booleanNumbers", "[-1,0.5]");
    CodecSettings settings = new CodecSettings(config);
    settings.init();
    @SuppressWarnings("unchecked")
  ...
  void should_throw_exception_when_booleanStrings_empty() {
    Config config = TestConfigUtils.createTestConfig("ksbulk.codec", "booleanStrings", "[]");
    CodecSettings settings = new CodecSettings(config);
    assertThatThrownBy(settings::init)
        .isInstanceOf(IllegalArgumentException.class)
        .hasMessageContaining(
            "Invalid value for ksbulk.codec.booleanStrings, expecting list with at least one true:false pair, got '[]'");
  }

  @Test
  void should_throw_exception_when_booleanStrings_not_a_list_of_tokens() {
    Config config =
        TestConfigUtils.createTestConfig("ksbulk.codec", "booleanStrings", "[NotATrueFalsePair]");
    CodecSettings settings = new CodecSettings(config);
    assertThatThrownBy(settings::init)
        .isInstanceOf(IllegalArgumentException.class)
        .hasMessageContaining(
            "Invalid value for ksbulk.codec.booleanStrings, expecting list with at least one true:false pair, got '[NotATrueFalsePair]'");
  }
  ...
  void should_create_settings_when_valid_booleanStrings() {
    Config config =
        TestConfigUtils.createTestConfig(
            "ksbulk.codec", "booleanStrings", "[\"VRAI:FAUX\",\"Σωστό:Λάθος\"]");
    CodecSettings settings = new CodecSettings(config);
    settings.init();
    @SuppressWarnings("unchecked")
  ...
  void should_create_settings_when_valid_hex_binary_format() {
    Config config = TestConfigUtils.createTestConfig("ksbulk.codec", "binary", "Hex");
    CodecSettings settings = new CodecSettings(config);
    settings.init();
  ...
  void should_create_settings_when_valid_base64_binary_format() {
    Config config = TestConfigUtils.createTestConfig("ksbulk.codec", "binary", "Base64");
    CodecSettings settings = new CodecSettings(config);
    settings.init();
    assertThat(ReflectionUtils.getInternalState(settings, "binaryFormat"))
  ...
  void should_throw_exception_when_invalid_binary_format() {
    Config config = TestConfigUtils.createTestConfig("ksbulk.codec", "binary", "NotABinaryFormat");
    CodecSettings settings = new CodecSettings(config);
    assertThatThrownBy(settings::init)
        .isInstanceOf(IllegalArgumentException.class)
        .hasMessageContaining(
            "Invalid value for ksbulk.codec.binary, expecting HEX or BASE64, got NotABinaryFormat");
  }
}
