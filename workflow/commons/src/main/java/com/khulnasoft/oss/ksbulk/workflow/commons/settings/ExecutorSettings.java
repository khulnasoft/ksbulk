...
public class ExecutorSettings {
  ...
  public void init() {
    try {
      ...
      throw ConfigUtils.convertConfigException(e, "ksbulk.executor");
    }
        ...
        if (ConfigUtils.hasUserOverride(config, "continuousPaging.maxConcurrentQueries")) {
          LOGGER.warn(
              "Setting executor.continuousPaging.maxConcurrentQueries has been removed and is not honored anymore; "
                  ...
                  + "--ksbulk.engine.maxConcurrentQueries instead.");
        }
      }
    } catch (ConfigException e) {
  ...
  public void enforceCloudRateLimit(int numberOfCoordinators) {
    if (ConfigUtils.hasReferenceValue(config, "maxPerSecond")) {
      ...
      LOGGER.info(
          "Setting executor.maxPerSecond not set when connecting to KhulnaSoft Astra: "
