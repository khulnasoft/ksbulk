...
import com.khulnasoft.dse.driver.api.core.auth.DseGssApiAuthProviderBase.GssApiOptions;
...
public class AuthProviderFactory {
    ...
    switch (authProvider.toLowerCase()) {
      ...
      default:
        throw new IllegalArgumentException(
            String.format(
                "Invalid value for ksbulk.driver.auth.provider, expecting one of "
  ...
  private static AuthProvider createGssApiAuthProvider(
    ...
    if (!config.hasPath("saslService")) {
      throw new IllegalArgumentException(
          String.format(
              "ksbulk.driver.auth.saslService must be provided with %s. "
                  + "ksbulk.driver.auth.principal, ksbulk.driver.auth.keyTab, and "
                  + "ksbulk.driver.auth.authorizationId are optional.",
              authProvider));
    }
  ...
  private static void checkHasCredentials(Config config, String authProvider) {
    if (!config.hasPath("username") || !config.hasPath("password")) {
      throw new IllegalArgumentException(
          "Both ksbulk.driver.auth.username and ksbulk.driver.auth.password must be provided with "
              + authProvider);
    }
  }
