...
import static com.khulnasoft.oss.ksbulk.batcher.api.BatchMode.PARTITION_KEY;
import static com.khulnasoft.oss.ksbulk.batcher.api.BatchMode.REPLICA_SET;
...
import com.khulnasoft.oss.ksbulk.batcher.reactor.ReactorStatementBatcher;
...
class BatchSettingsTest {
  ...
  void should_create_batcher_when_mode_is_default() {
    Config config = TestConfigUtils.createTestConfig("ksbulk.batch");
  ...
  void should_create_batcher_when_mode_is_default_for_new_maxBatchStatements() {
    Config config =
        TestConfigUtils.createTestConfig(
            "ksbulk.batch", "maxBatchStatements", 32, "bufferSize", 32, "mode", "PARTITION_KEY");
    BatchSettings settings = new BatchSettings(config);
    settings.init();
    assertThat(settings.getBufferSize()).isEqualTo(32);
    ReactiveStatementBatcher batcher = settings.newStatementBatcher(session);
  ...
  void should_create_batcher_when_batch_mode_provided() {
    Config config = TestConfigUtils.createTestConfig("ksbulk.batch", "mode", "REPLICA_SET");
    BatchSettings settings = new BatchSettings(config);
    settings.init();
    assertThat(settings.getBufferSize()).isEqualTo(128);
    ReactiveStatementBatcher batcher = settings.newStatementBatcher(session);
  ...
  void should_create_batcher_when_buffer_size_provided() {
    Config config = TestConfigUtils.createTestConfig("ksbulk.batch", "bufferSize", 5000);
    BatchSettings settings = new BatchSettings(config);
    settings.init();
  ...
  void should_create_batcher_when_max_batch_statements_mode_provided() {
    Config config =
        TestConfigUtils.createTestConfig(
            "ksbulk.batch", "maxBatchStatements", 10, "mode", "PARTITION_KEY", "bufferSize", -1);
    BatchSettings settings = new BatchSettings(config);
    settings.init();
  ...
  void should_throw_exception_when_buffer_size_less_than_max_batch_statements() {
    Config config =
        TestConfigUtils.createTestConfig(
            "ksbulk.batch", "maxBatchStatements", 10, "bufferSize", 5, "mode", "PARTITION_KEY");
    BatchSettings settings = new BatchSettings(config);
    assertThatThrownBy(settings::init)
        .isInstanceOf(IllegalArgumentException.class)
        .hasMessage(
  ...
  void should_throw_exception_when_max_batch_size_not_a_number() {
    Config config =
        TestConfigUtils.createTestConfig(
            "ksbulk.batch", "maxBatchSize", "NotANumber", "maxBatchStatements", "null");
    BatchSettings settings = new BatchSettings(config);
    assertThatThrownBy(settings::init)
        .isInstanceOf(IllegalArgumentException.class)
        .hasMessageContaining(
            "Invalid value for ksbulk.batch.maxBatchSize, expecting NUMBER, got STRING");
  }

  @Test
  void should_throw_exception_when_max_batch_statements_not_a_number() {
    Config config =
        TestConfigUtils.createTestConfig("ksbulk.batch", "maxBatchStatements", "NotANumber");
    BatchSettings settings = new BatchSettings(config);
    assertThatThrownBy(settings::init)
        .isInstanceOf(IllegalArgumentException.class)
        .hasMessageContaining(
            "Invalid value for ksbulk.batch.maxBatchStatements, expecting NUMBER, got STRING");
  }

  @Test
  void should_throw_exception_when_max_size_in_bytes_not_a_number() {
    Config config =
        TestConfigUtils.createTestConfig("ksbulk.batch", "maxSizeInBytes", "NotANumber");
    BatchSettings settings = new BatchSettings(config);
    assertThatThrownBy(settings::init)
        .isInstanceOf(IllegalArgumentException.class)
        .hasMessageContaining(
            "Invalid value for ksbulk.batch.maxSizeInBytes, expecting NUMBER or STRING in size-in-bytes format, got 'NotANumber'");
  }

  @Test
  void should_throw_exception_when_buffer_size_not_a_number() {
    Config config = TestConfigUtils.createTestConfig("ksbulk.batch", "bufferSize", "NotANumber");
    BatchSettings settings = new BatchSettings(config);
    assertThatThrownBy(settings::init)
        .isInstanceOf(IllegalArgumentException.class)
        .hasMessageContaining(
            "Invalid value for ksbulk.batch.bufferSize, expecting NUMBER, got STRING");
  }

  @Test
  void should_throw_exception_when_batch_mode_invalid() {
    Config config = TestConfigUtils.createTestConfig("ksbulk.batch", "mode", "NotAMode");
    BatchSettings settings = new BatchSettings(config);
    assertThatThrownBy(settings::init)
        .isInstanceOf(IllegalArgumentException.class)
        .hasMessageContaining(
            "Invalid value for ksbulk.batch.mode, expecting one of DISABLED, PARTITION_KEY, REPLICA_SET, got: 'NotAMode'");
  }
  ...
  void should_throw_exception_when_max_batch_statements_and_max_size_in_bytes_are_non_positive() {
    Config config =
        TestConfigUtils.createTestConfig(
            "ksbulk.batch", "mode", "PARTITION_KEY", "maxBatchStatements", -1, "maxSizeInBytes", 0);
    BatchSettings settings = new BatchSettings(config);
    assertThatThrownBy(settings::init)
        .isInstanceOf(IllegalArgumentException.class)
        .hasMessageContaining(
