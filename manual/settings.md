# KhulnaSoft Bulk Loader v1.11.0-SNAPSHOT Options
...
<a href="#khulnasoft-java-driver">Driver Settings</a><br>
<a name="Common"></a>
## Common Settings
...
#### -c,<br />--connector.name<br />--ksbulk.connector.name _&lt;string&gt;_

The name of the connector to use.

Default: **"csv"**.

#### -url,<br />--connector.csv.url<br />--ksbulk.connector.csv.url _&lt;string&gt;_
...
#### -delim,<br />--connector.csv.delimiter<br />--ksbulk.connector.csv.delimiter _&lt;string&gt;_

The character(s) to use as field delimiter. Field delimiters containing more than one character are accepted.

Default: **","**.

#### -header,<br />--connector.csv.header<br />--ksbulk.connector.csv.header _&lt;boolean&gt;_
...
#### -skipRecords,<br />--connector.csv.skipRecords<br />--ksbulk.connector.csv.skipRecords _&lt;number&gt;_
...
#### -maxRecords,<br />--connector.csv.maxRecords<br />--ksbulk.connector.csv.maxRecords _&lt;number&gt;_
...
#### -url,<br />--connector.json.url<br />--ksbulk.connector.json.url _&lt;string&gt;_
...
#### -skipRecords,<br />--connector.json.skipRecords<br />--ksbulk.connector.json.skipRecords _&lt;number&gt;_
...
#### -maxRecords,<br />--connector.json.maxRecords<br />--ksbulk.connector.json.maxRecords _&lt;number&gt;_
...
#### -h,<br />--driver.basic.contact-points<br />--khulnasoft-java-driver.basic.contact-points _&lt;list&lt;string&gt;&gt;_
...
The heuristic to determine whether or not a contact point is in the form "host" or "host:port" is not 100% accurate for some IPv6 addresses; you should avoid ambiguous IPv6 addresses such as `fe80::f861:3eff:fe1d:1234`, because such a string can be seen either as a combination of IP `fe80::f861:3eff:fe1d` with port 1234, or as IP `fe80::f861:3eff:fe1d:1234` without port. In such cases, KSBulk will not change the contact point. This issue can be avoided by providing IPv6 addresses in full form, e.g. if instead of `fe80::f861:3eff:fe1d:1234` you provide `fe80:0:0:0:0:f861:3eff:fe1d:1234`, then the string is unequivocally parsed as IP `fe80:0:0:0:0:f861:3eff:fe1d` with port 1234.

Note: on Cloud deployments, KSBulk automatically sets this option to an empty list, as contact points are not allowed to be explicitly provided when connecting to KhulnaSoft Astra databases.

Default: **["127.0.0.1:9042"]**.

#### -port,<br />--driver.basic.default-port<br />--khulnasoft-java-driver.basic.default-port _&lt;number&gt;_
...
#### -b,<br />--driver.basic.cloud.secure-connect-bundle<br />--khulnasoft-java-driver.basic.cloud.secure-connect-bundle _&lt;string&gt;_

The location of the secure bundle used to connect to a Khulnasoft Astra database. This setting must be a path on the local filesystem or a valid URL.
...
Note: if you set this to a non-null value, KSBulk assumes that you are connecting to an KhulnaSoft Astra database; in this case, you should not set any of the following settings because they are not compatible with Cloud deployments:

- `khulnasoft-java-driver.basic.contact-points`
- `khulnasoft-java-driver.basic.request.consistency`
- `khulnasoft-java-driver.advanced.ssl-engine-factory.*`

If you do so, a log will be emitted and the setting will be ignored.

Default: **null**.

#### -u,<br />--driver.advanced.auth-provider.username<br />--khulnasoft-java-driver.advanced.auth-provider.username _&lt;string&gt;_
...
#### -p,<br />--driver.advanced.auth-provider.password<br />--khulnasoft-java-driver.advanced.auth-provider.password _&lt;string&gt;_
...
#### -cl,<br />--driver.basic.request.consistency<br />--khulnasoft-java-driver.basic.request.consistency _&lt;string&gt;_
...
#### -dc,<br />--driver.basic.load-balancing-policy.local-datacenter<br />--khulnasoft-java-driver.basic.load-balancing-policy.local-datacenter _&lt;string&gt;_

The datacenter that is considered "local": the default load balancing policy will only include nodes from this datacenter in its query plans. Set this to a non-null value if you want to force the local datacenter; otherwise, the `DcInferringLoadBalancingPolicy` used by default by KSBulk will infer the local datacenter from the provided contact points.

Default: **null**.

#### -k,<br />--schema.keyspace<br />--ksbulk.schema.keyspace _&lt;string&gt;_
...
#### -t,<br />--schema.table<br />--ksbulk.schema.table _&lt;string&gt;_
...
#### -m,<br />--schema.mapping<br />--ksbulk.schema.mapping _&lt;string&gt;_
...
#### -dryRun,<br />--engine.dryRun<br />--ksbulk.engine.dryRun _&lt;boolean&gt;_
...
#### -maxConcurrentQueries,<br />--engine.maxConcurrentQueries<br />--ksbulk.engine.maxConcurrentQueries _&lt;string&gt;_
...
The default value is 'AUTO'; with this special value, KSBulk will optimize the number of concurrent queries according to the number of available cores, and the operation being executed. The actual value usually ranges from the number of cores to eight times that number.

Default: **"AUTO"**.

#### -maxErrors,<br />--log.maxErrors<br />--ksbulk.log.maxErrors _&lt;number&gt;_
...
#### -logDir,<br />--log.directory<br />--ksbulk.log.directory _&lt;string&gt;_
...
#### -verbosity,<br />--log.verbosity<br />--ksbulk.log.verbosity _&lt;string&gt;_

The desired level of verbosity. Valid values are:

- `quiet`: KSBulk will only log WARN and ERROR messages.
- `normal`: KSBulk will log a few INFO messages, as well as WARN and ERROR messages.
- `high`: KSBulk will also print some DEBUG messages at the beginning of the operation, such as KSBulk's settings, inferred query, and the read and write concurrency.
- `max`: KSBulk will print numerous DEBUG messages from itself, the driver, and from some important libraries (Netty and Reactor).
...
#### -reportRate,<br />--monitoring.reportRate<br />--ksbulk.monitoring.reportRate _&lt;string&gt;_

The report interval. KSBulk will print useful metrics about the ongoing operation at this rate; for example, if this value is set to 10 seconds, then KSBulk will print metrics every ten seconds. Valid values: any value specified in [HOCON duration syntax](https://github.com/lightbend/config/blob/master/HOCON.md#duration-format), but durations lesser than one second will be rounded up to 1 second.
...
#### -quote,<br />--connector.csv.quote<br />--ksbulk.connector.csv.quote _&lt;string&gt;_
...
#### -comment,<br />--connector.csv.comment<br />--ksbulk.connector.csv.comment _&lt;string&gt;_
...
#### --connector.csv.compression<br />--ksbulk.connector.csv.compression _&lt;string&gt;_
...
#### --connector.csv.emptyValue<br />--ksbulk.connector.csv.emptyValue _&lt;string&gt;_
...
#### -encoding,<br />--connector.csv.encoding<br />--ksbulk.connector.csv.encoding _&lt;string&gt;_

The file encoding to use for all read or written files.

Default: **"UTF-8"**.

#### -escape,<br />--connector.csv.escape<br />--ksbulk.connector.csv.escape _&lt;string&gt;_
...
#### --connector.csv.fileNameFormat<br />--ksbulk.connector.csv.fileNameFormat _&lt;string&gt;_
...
#### --connector.csv.fileNamePattern<br />--ksbulk.connector.csv.fileNamePattern _&lt;string&gt;_
...
#### --connector.csv.ignoreLeadingWhitespaces<br />--ksbulk.connector.csv.ignoreLeadingWhitespaces _&lt;boolean&gt;_
...
#### --connector.csv.ignoreLeadingWhitespacesInQuotes<br />--ksbulk.connector.csv.ignoreLeadingWhitespacesInQuotes _&lt;boolean&gt;_
...
#### --connector.csv.ignoreTrailingWhitespaces<br />--ksbulk.connector.csv.ignoreTrailingWhitespaces _&lt;boolean&gt;_
...
#### --connector.csv.ignoreTrailingWhitespacesInQuotes<br />--ksbulk.connector.csv.ignoreTrailingWhitespacesInQuotes _&lt;boolean&gt;_
...
#### --connector.csv.maxCharsPerColumn<br />--ksbulk.connector.csv.maxCharsPerColumn _&lt;number&gt;_
...
#### --connector.csv.maxColumns<br />--ksbulk.connector.csv.maxColumns _&lt;number&gt;_
...
#### -maxConcurrentFiles,<br />--connector.csv.maxConcurrentFiles<br />--ksbulk.connector.csv.maxConcurrentFiles _&lt;string&gt;_
...
#### -newline,<br />--connector.csv.newline<br />--ksbulk.connector.csv.newline _&lt;string&gt;_
...
#### --connector.csv.normalizeLineEndingsInQuotes<br />--ksbulk.connector.csv.normalizeLineEndingsInQuotes _&lt;boolean&gt;_
...
#### --connector.csv.nullValue<br />--ksbulk.connector.csv.nullValue _&lt;string&gt;_
...
#### --connector.csv.recursive<br />--ksbulk.connector.csv.recursive _&lt;boolean&gt;_
...
#### --connector.csv.urlfile<br />--ksbulk.connector.csv.urlfile _&lt;string&gt;_
...
#### --connector.json.mode<br />--ksbulk.connector.json.mode _&lt;string&gt;_
...
#### --connector.json.compression<br />--ksbulk.connector.json.compression _&lt;string&gt;_

The compression that will be used for writing or reading files. Supported values are (for both reading and writing): `none`, `xz`, `gzip`, `bzip2`, `zstd`, `lz4`, `lzma`, `snappy`, `deflate`.  For reading only, supported values are: `brotli`, `z`, `deflate64`.

Default: **"none"**.

#### --connector.json.deserializationFeatures<br />--ksbulk.connector.json.deserializationFeatures _&lt;map&lt;string,boolean&gt;&gt;_
...
Note that some Jackson features might not be supported, in particular features that operate on the resulting Json tree by filtering elements or altering their contents, since such features conflict with ksbulk's own filtering and formatting capabilities. Instead of trying to modify the resulting tree using Jackson features, you should try to achieve the same result using the settings available under the `codec` and `schema` sections.

#### -encoding,<br />--connector.json.encoding<br />--ksbulk.connector.json.encoding _&lt;string&gt;_

The file encoding to use for all read or written files.

Default: **"UTF-8"**.

#### --connector.json.fileNameFormat<br />--ksbulk.connector.json.fileNameFormat _&lt;string&gt;_
...
#### --connector.json.fileNamePattern<br />--ksbulk.connector.json.fileNamePattern _&lt;string&gt;_
...
#### --connector.json.generatorFeatures<br />--ksbulk.connector.json.generatorFeatures _&lt;map&lt;string,boolean&gt;&gt;_
...
#### -maxConcurrentFiles,<br />--connector.json.maxConcurrentFiles<br />--ksbulk.connector.json.maxConcurrentFiles _&lt;string&gt;_
...
#### --connector.json.parserFeatures<br />--ksbulk.connector.json.parserFeatures _&lt;map&lt;string,boolean&gt;&gt;_
...
#### --connector.json.prettyPrint<br />--ksbulk.connector.json.prettyPrint _&lt;boolean&gt;_
...
#### --connector.json.recursive<br />--ksbulk.connector.json.recursive _&lt;boolean&gt;_

Enable or disable scanning for files in the root's subdirectories. Only applicable when *url* is set to a directory on a known filesystem. Used for loading only.

Default: **false**.

#### --connector.json.serializationFeatures<br />--ksbulk.connector.json.serializationFeatures _&lt;map&lt;string,boolean&gt;&gt;_
...
#### --connector.json.serializationStrategy<br />--ksbulk.connector.json.serializationStrategy _&lt;string&gt;_
...
#### --connector.json.urlfile<br />--ksbulk.connector.json.urlfile _&lt;string&gt;_
...
#### --schema.allowExtraFields<br />--ksbulk.schema.allowExtraFields _&lt;boolean&gt;_
...
#### --schema.allowMissingFields<br />--ksbulk.schema.allowMissingFields _&lt;boolean&gt;_
...
#### -e,<br />--schema.edge<br />--ksbulk.schema.edge _&lt;string&gt;_
...
#### -from,<br />--schema.from<br />--ksbulk.schema.from _&lt;string&gt;_
...
#### -g,<br />--schema.graph<br />--ksbulk.schema.graph _&lt;string&gt;_
...
#### --schema.nullToUnset<br />--ksbulk.schema.nullToUnset _&lt;boolean&gt;_
...
#### -timestamp,<br />--schema.preserveTimestamp<br />--ksbulk.schema.preserveTimestamp _&lt;boolean&gt;_
...
For both loading and unlaoding, KSBulk will import and export TTLs using field names such as `"ttl(<column>)"`, where `<column>` is the column's internal CQL name; for example, if the table has a column named `"MyCol"`, its corresponding TTL would be exported as `"ttl(MyCol)"` in the generated query and in the resulting connector record. If you intend to use this feature to export and import tables letting KSBulk generate the appropriate queries, these names are fine and need not be changed. If, however, you would like to export or import data to or from external sources that use different field names, you could do so by using the function `ttl` in a schema.mapping entry; for example, the following mapping would map `col1` along with its TTL to two distinct fields, `field1` and `field1_ttl`: `field1 = col1, field1_ttl = ttl(col1)`.

Default: **false**.

#### -query,<br />--schema.query<br />--ksbulk.schema.query _&lt;string&gt;_

The query to use. If not specified, then *schema.keyspace* and *schema.table* must be specified, and ksbulk will infer the appropriate statement based on the table's metadata, using all available columns. If `schema.keyspace` is provided, the query need not include the keyspace to qualify the table reference.
...
#### --schema.queryTimestamp<br />--ksbulk.schema.queryTimestamp _&lt;string&gt;_
...
Query timestamps for Cassandra have microsecond resolution; any sub-microsecond information specified is lost. For more information, see the [CQL Reference](https://docs.khulnasoft.com/en/dse/6.0/cql/cql/cql_reference/cql_commands/cqlInsert.html#cqlInsert__timestamp-value).

Default: **null**.

#### --schema.queryTtl<br />--ksbulk.schema.queryTtl _&lt;number&gt;_

The Time-To-Live (TTL) of inserted/updated cells during load (seconds); a value of -1 means there is no TTL. Not applicable to unloading nor counting. Ignored when `schema.query` is provided. For more information, see the [CQL Reference](https://docs.khulnasoft.com/en/dse/6.0/cql/cql/cql_reference/cql_commands/cqlInsert.html#cqlInsert__ime-value), [Setting the time-to-live (TTL) for value](http://docs.khulnasoft.com/en/dse/6.0/cql/cql/cql_using/useTTL.html), and [Expiring data with time-to-live](http://docs.khulnasoft.com/en/dse/6.0/cql/cql/cql_using/useExpire.html).

Default: **-1**.

#### --schema.splits<br />--ksbulk.schema.splits _&lt;string&gt;_
...
#### -to,<br />--schema.to<br />--ksbulk.schema.to _&lt;string&gt;_
...
#### -v,<br />--schema.vertex<br />--ksbulk.schema.vertex _&lt;string&gt;_
...
#### --batch.bufferSize<br />--ksbulk.batch.bufferSize _&lt;number&gt;_
...
#### --batch.maxBatchSize<br />--ksbulk.batch.maxBatchSize _&lt;number&gt;_

**DEPRECATED**. Use `maxBatchStatements` instead.

Default: **null**.

#### --batch.maxBatchStatements<br />--ksbulk.batch.maxBatchStatements _&lt;number&gt;_
...
#### --batch.maxSizeInBytes<br />--ksbulk.batch.maxSizeInBytes _&lt;number&gt;_
...
#### --batch.mode<br />--ksbulk.batch.mode _&lt;string&gt;_
...
When tuning KSBulk for batching, the recommended approach is as follows:
...
#### --codec.binary<br />--ksbulk.codec.binary _&lt;string&gt;_
...
#### --codec.booleanNumbers<br />--ksbulk.codec.booleanNumbers _&lt;list&lt;number&gt;&gt;_
...
#### --codec.booleanStrings<br />--ksbulk.codec.booleanStrings _&lt;list&lt;string&gt;&gt;_

Specify how true and false representations can be used by ksbulk. Each representation is of the form `true_value:false_value`, case-insensitive. For loading, all representations are honored: when a record field value exactly matches one of the specified strings, the value is replaced with `true` of `false` before writing to the database. For unloading, this setting is only applicable for string-based connectors, such as the CSV connector: the first representation will be used to format booleans before they are written out, and all others are ignored.

Default: **["1:0","Y:N","T:F","YES:NO","TRUE:FALSE"]**.

#### --codec.date<br />--ksbulk.codec.date _&lt;string&gt;_
...
For more information about CQL date, time and timestamp literals, see [Date, time, and timestamp format](https://docs.khulnasoft.com/en/dse/6.0/cql/cql/cql_reference/refDateTimeFormats.html?hl=timestamp).

Default: **"ISO_LOCAL_DATE"**.

#### --codec.epoch<br />--ksbulk.codec.epoch _&lt;string&gt;_
...
#### --codec.formatNumbers<br />--ksbulk.codec.formatNumbers _&lt;boolean&gt;_
...
#### --codec.geo<br />--ksbulk.codec.geo _&lt;string&gt;_

Strategy to use when converting geometry types to strings. Geometry types are only available in KhulnaSoft Enterprise (DSE) 5.0 or higher. Only applicable when unloading columns of CQL type `Point`, `LineString` or `Polygon`, and only if the connector in use requires stringification. Valid values are:
...
#### -locale,<br />--codec.locale<br />--ksbulk.codec.locale _&lt;string&gt;_

The locale to use for locale-sensitive conversions.

Default: **"en_US"**.

#### -nullStrings,<br />--codec.nullStrings<br />--ksbulk.codec.nullStrings _&lt;list&gt;_
...
The default value is `[]` (no strings are mapped to `null`). In the default mode, KSBulk behaves as follows:
...
Note that, regardless of this setting, KSBulk will always convert empty strings to `null` if the target CQL type is not textual when loading (i.e. not text, varchar or ascii).
...
#### --codec.number<br />--ksbulk.codec.number _&lt;string&gt;_
...
#### --codec.overflowStrategy<br />--ksbulk.codec.overflowStrategy _&lt;string&gt;_
...
#### --codec.roundingStrategy<br />--ksbulk.codec.roundingStrategy _&lt;string&gt;_
...
#### --codec.time<br />--ksbulk.codec.time _&lt;string&gt;_
...
#### -timeZone,<br />--codec.timeZone<br />--ksbulk.codec.timeZone _&lt;string&gt;_
...
#### --codec.timestamp<br />--ksbulk.codec.timestamp _&lt;string&gt;_
...
#### --codec.unit<br />--ksbulk.codec.unit _&lt;string&gt;_
...
#### --codec.uuidStrategy<br />--ksbulk.codec.uuidStrategy _&lt;string&gt;_
...
#### --engine.dataSizeSamplingEnabled<br />--ksbulk.engine.dataSizeSamplingEnabled _&lt;boolean&gt;_

Specify whether KSBulk should use data size sampling to optimize its execution engine. Only applicable for loading, ignored otherwise.
...
Note that when loading from standard input, KSBulk will never perform data size sampling, regardless of the value set here.

The default value is 'true', meaning that data size sampling is enabled.

Default: **true**.

#### --engine.executionId<br />--ksbulk.engine.executionId _&lt;string&gt;_
...
Executor-specific settings. Executor settings control how the KhulnaSoft Java driver is used by KSBulk, and notably, the desired amount of driver-level concurrency and throughput. These settings are for advanced users.

#### --executor.continuousPaging.enabled<br />--ksbulk.executor.continuousPaging.enabled _&lt;boolean&gt;_
...
#### --executor.continuousPaging.maxConcurrentQueries<br />--ksbulk.executor.continuousPaging.maxConcurrentQueries _&lt;number&gt;_
...
#### --executor.continuousPaging.maxPages<br />--ksbulk.executor.continuousPaging.maxPages _&lt;number&gt;_

The maximum number of pages to retrieve. Setting this value to zero retrieves all pages available.

**DEPRECATED**. Use `khulnasoft-java-driver.advanced.continuous-paging.max-pages` instead.

Default: **0**.

#### --executor.continuousPaging.maxPagesPerSecond<br />--ksbulk.executor.continuousPaging.maxPagesPerSecond _&lt;number&gt;_

The maximum number of pages per second. Setting this value to zero indicates no limit.

**DEPRECATED**. Use `khulnasoft-java-driver.advanced.continuous-paging.max-pages-per-second` instead.

Default: **0**.

#### --executor.continuousPaging.pageSize<br />--ksbulk.executor.continuousPaging.pageSize _&lt;number&gt;_
...
**DEPRECATED**. Use `khulnasoft-java-driver.advanced.continuous-paging.page-size` instead.

Default: **5000**.

#### --executor.continuousPaging.pageUnit<br />--ksbulk.executor.continuousPaging.pageUnit _&lt;string&gt;_

The unit to use for the `pageSize` setting. Possible values are: `ROWS`, `BYTES`.

**DEPRECATED**. Use `khulnasoft-java-driver.advanced.continuous-paging.page-size-in-bytes` instead.

Default: **"ROWS"**.

#### --executor.maxBytesPerSecond<br />--ksbulk.executor.maxBytesPerSecond _&lt;number&gt;_
...
#### --executor.maxInFlight<br />--ksbulk.executor.maxInFlight _&lt;number&gt;_
...
#### --executor.maxPerSecond<br />--ksbulk.executor.maxPerSecond _&lt;number&gt;_
...
When connecting to KhulnaSoft Astra clusters, a rate limit is always enforced. By default, the limit is 4,096 operations per second and per coordinator; therefore, if no rate limit is set, and the cluster is an Astra cluster, KSBulk will automatically apply an appropriate limit and log a message.
...
#### --log.ansiMode<br />--ksbulk.log.ansiMode _&lt;string&gt;_

Whether or not to use ANSI colors and other escape sequences in log messages printed to the console. Valid values are:

- `normal`: this is the default option. KSBulk will only use ANSI when the terminal is:
...
- `force`: KSBulk will use ANSI, even if the terminal has not been detected as ANSI-compatible.
- `disabled`: KSBulk will not use ANSI.

Default: **"normal"**.

#### --log.checkpoint.enabled<br />--ksbulk.log.checkpoint.enabled _&lt;boolean&gt;_

Whether to enable checkpointing for the current operation. When set to true (the default), KSBulk will track records that were processed and will produce a checkpoint file at the end of the operation. The checkpoint file can then be used later on to resume the same operation, if not all records were processed, or if the operation was interrupted.
...
#### --log.checkpoint.file<br />--ksbulk.log.checkpoint.file _&lt;string&gt;_
...
#### --log.checkpoint.replayStrategy<br />--ksbulk.log.checkpoint.replayStrategy _&lt;string&gt;_

The replay strategy to use when resuming an operation from a checkpoint file. Valid values are:
- `resume`: KSBulk will only process new records from resources that weren't consumed entirely. Records that were already processed will be ignored, including rejected ones (rejected records are always written to bad files). This is the safest option when loading if the operation is not idempotent.
- `retry`: this is the default option. KSBulk will process new and rejected records from resources that weren't consumed entirely. Note that this strategy may result in some rows being inserted twice and thus should only be used if the operation is idempotent.
- `retryAll`: like `retry`, KSBulk will process new and rejected records, but unlike `retry`, it will process all resources, including those marked as consumed entirely. Note that this strategy may result in some rows being inserted twice and thus should only be used if the operation is idempotent.

Default: **"retry"**.

#### --log.maxQueryWarnings<br />--ksbulk.log.maxQueryWarnings _&lt;number&gt;_

The maximum number of query warnings to log before muting them. Query warnings are sent by the server (for example, if the number of statements in a batch is greater than the warning threshold configured on the server). They are useful to diagnose suboptimal configurations but tend to be too invasive, which is why KSBulk by default will only log the 50 first query warnings; any subsequent warnings will be muted and won't be logged at all. Setting this value to any negative integer disables this feature (not recommended).

Default: **50**.

#### --log.row.maxResultSetValueLength<br />--ksbulk.log.row.maxResultSetValueLength _&lt;number&gt;_
...
#### --log.row.maxResultSetValues<br />--ksbulk.log.row.maxResultSetValues _&lt;number&gt;_
...
#### --log.sources<br />--ksbulk.log.sources _&lt;boolean&gt;_
...
#### --log.stmt.maxBoundValueLength<br />--ksbulk.log.stmt.maxBoundValueLength _&lt;number&gt;_
...
#### --log.stmt.maxBoundValues<br />--ksbulk.log.stmt.maxBoundValues _&lt;number&gt;_
...
#### --log.stmt.maxInnerStatements<br />--ksbulk.log.stmt.maxInnerStatements _&lt;number&gt;_
...
#### --log.stmt.maxQueryStringLength<br />--ksbulk.log.stmt.maxQueryStringLength _&lt;number&gt;_
...
#### --monitoring.console<br />--ksbulk.monitoring.console _&lt;boolean&gt;_

Enable or disable console reporting. If enabled, KSBulk will print useful metrics about the ongoing operation to standard error; the metrics will be refreshed at `reportRate`. Displayed information includes: total records, failed records, throughput, latency, and if available, average batch size. Note that when `log.verbosity` is set to quiet (0), KSBulk will disable the console reporter regardless of the value specified here. The default is true (print ongoing metrics to the console).

Default: **true**.

#### --monitoring.csv<br />--ksbulk.monitoring.csv _&lt;boolean&gt;_
...
#### --monitoring.durationUnit<br />--ksbulk.monitoring.durationUnit _&lt;string&gt;_
...
#### --monitoring.expectedReads<br />--ksbulk.monitoring.expectedReads _&lt;number&gt;_
...
#### --monitoring.expectedWrites<br />--ksbulk.monitoring.expectedWrites _&lt;number&gt;_
...
#### -jmx,<br />--monitoring.jmx<br />--ksbulk.monitoring.jmx _&lt;boolean&gt;_

Enable or disable JMX reporting. Note that to enable remote JMX reporting, several properties must also be set in the JVM during launch. This is accomplished via the `KSBULK_JAVA_OPTS` environment variable.
...
- `khulnasoft-java-driver.advanced.metrics.session.enabled` should contain a list of session-level metric names to enable;
- `khulnasoft-java-driver.advanced.metrics.node.enabled` should contain a list of node-level metric names to enable.
Driver metrics appear under a folder named after the session name (by default in KSBulk, the session name is simply "driver").

Default: **true**.

#### --monitoring.prometheus.job<br />--ksbulk.monitoring.prometheus.job _&lt;string&gt;_

The job name to use. KSBulk will add a label "job" with this value to each exported metric. This is also the job name used when pushing metrics to a PushGateway. The default is "KSBulk". See https://github.com/prometheus/pushgateway#about-the-job-and-instance-labels for more information.

Default: **"KSBulk"**.

#### --monitoring.prometheus.labels<br />--ksbulk.monitoring.prometheus.labels _&lt;map&lt;string,string&gt;&gt;_

A set of static labels to add to each exported metric, in both pull and push modes. Note that KSBulk automatically adds the following labels:

- `operation_id` is set to the current operation ID (a.k.a. execution ID, see `engine.executionId`);
- `job` is set to the job name (as defined by `monitoring.prometheus.job`, by default "KSBulk");
- `application_name` is set to "KhulnaSoft Bulk Loader" followed by the operation ID;
- `application_version` is set to the KSBulk's version;
- `driver_version` is set to the KhulnaSoft Java driver version;
- `client_id` is set to KSBulk's client UUID.

The last four labels correspond to values that KSBulk also passes to the driver, which in turn uses the same info to connect to Cassandra. This makes it possible to correlate data sent by the driver to Cassandra with data sent by KSBulk to Prometheus.

#### -prometheus,<br />--monitoring.prometheus.pull.enabled<br />--ksbulk.monitoring.prometheus.pull.enabled _&lt;boolean&gt;_

Enable or disable exposing metrics to Prometheus in the traditional pull model (scraping). If enabled, all KSBulk and metrics will be accessible at an (unsecured) HTTP endpoint. Driver metrics can also be exported, but they are disabled by default; see `monitoring.jmx` for details about how to enable them.

Default: **false**.

#### --monitoring.prometheus.pull.hostname<br />--ksbulk.monitoring.prometheus.pull.hostname _&lt;string&gt;_
...
#### --monitoring.prometheus.pull.port<br />--ksbulk.monitoring.prometheus.pull.port _&lt;number&gt;_

The port that the metrics HTTP server should bind to.

Default: **8080**.

#### --monitoring.prometheus.push.enabled<br />--ksbulk.monitoring.prometheus.push.enabled _&lt;boolean&gt;_

Enabled or disable pushing metrics to a PushGateway. If enabled, KSBulk will push metrics to this URL at the end of the operation. Note that not all metrics are exported when pushing to a PushGateway; only some high-level ones are, including the total time elapsed, the number of records processed and the number of rows written or read. In particular, driver metrics are currently not pushed.

Default: **false**.

#### --monitoring.prometheus.push.groupBy.instance<br />--ksbulk.monitoring.prometheus.push.groupBy.instance _&lt;boolean&gt;_

Whether to add an instance grouping key to exported metrics. If enabled, KSBulk adds an "instance" grouping key with its value set to the machine's IP address to the exported metrics. This will effectively group metrics by instance, rather than by job. See https://github.com/prometheus/pushgateway#about-the-job-and-instance-labels for more information.

Default: **false**.

#### --monitoring.prometheus.push.groupBy.keys<br />--ksbulk.monitoring.prometheus.push.groupBy.keys _&lt;map&lt;string,string&gt;&gt;_
...
#### --monitoring.prometheus.push.groupBy.operation<br />--ksbulk.monitoring.prometheus.push.groupBy.operation _&lt;boolean&gt;_

Whether to add the operation ID as a grouping key to exported metrics. If enabled, KSBulk adds an "operation_id" grouping key with its value set to the current operation ID (see `engine.executionId`) to the exported metrics. This will effectively group metrics by operation, rather than by job.

Default: **false**.

#### --monitoring.prometheus.push.password<br />--ksbulk.monitoring.prometheus.push.password _&lt;string&gt;_
...
#### --monitoring.prometheus.push.url<br />--ksbulk.monitoring.prometheus.push.url _&lt;string&gt;_
...
#### --monitoring.prometheus.push.username<br />--ksbulk.monitoring.prometheus.push.username _&lt;string&gt;_
...
#### --monitoring.rateUnit<br />--ksbulk.monitoring.rateUnit _&lt;string&gt;_
...
Runner-specific settings. Runner settings control how KSBulk parses command lines and reads its configuration.

#### --runner.promptForPasswords<br />--ksbulk.runner.promptForPasswords _&lt;boolean&gt;_

Whether to prompt for passwords when they are missing from configuration files. When this option is true (the default value), if a login or username is present in the configuration, but not its corresponding password, KSBulk will prompt for it.

Prompting from passwords require interactive shells; if the standard input is not connected to a terminal, no passwords will be prompted, even if this option is true. You should only disable this feature if KSBulk mistankenly assumes that it is running in an interactive shell.
...
#### --s3.clientCacheSize<br />--ksbulk.s3.clientCacheSize _&lt;number&gt;_
...
#### -stats,<br />--stats.modes<br />--ksbulk.stats.modes _&lt;list&lt;string&gt;&gt;_
...
#### -partitions,<br />--stats.numPartitions<br />--ksbulk.stats.numPartitions _&lt;number&gt;_
...
<a name="khulnasoft-java-driver"></a>
## Driver Settings

The settings below are just a subset of all the configurable options of the driver, and provide an optimal driver configuration for KSBulk for most use cases.

See the [Java Driver configuration reference](https://docs.khulnasoft.com/en/developer/java-driver/latest/manual/core/configuration) for instructions on how to configure the driver properly.

Note: driver settings always start with prefix `khulnasoft-java-driver`; on the command line only, it is possible to abbreviate this prefix to just `driver`, as shown below.

#### --driver.basic.session-name<br />--khulnasoft-java-driver.basic.session-name _&lt;string&gt;_

The driver session name. KSBulk simply calls it "driver". The session name is printed by all driver log messages, between square brackets.
...
#### --driver.basic.request.timeout<br />--khulnasoft-java-driver.basic.request.timeout _&lt;string&gt;_

How long the driver waits for a request to complete. This is a global limit on the duration of a session.execute() call, including any internal retries the driver might do. By default, this value is set very high because KSBulk is optimized for good throughput, rather than good latencies.
...
#### --driver.basic.request.serial-consistency<br />--khulnasoft-java-driver.basic.request.serial-consistency _&lt;string&gt;_

The serial consistency level. The allowed values are `SERIAL` and `LOCAL_SERIAL`.

Default: **"LOCAL_SERIAL"**.

#### --driver.basic.request.default-idempotence<br />--khulnasoft-java-driver.basic.request.default-idempotence _&lt;boolean&gt;_

The default idempotence for all queries executed in KSBulk. Setting this to false will cause all write failures to not be retried.

Default: **true**.

#### --driver.basic.request.page-size<br />--khulnasoft-java-driver.basic.request.page-size _&lt;number&gt;_
...
#### --driver.basic.load-balancing-policy.class<br />--khulnasoft-java-driver.basic.load-balancing-policy.class _&lt;string&gt;_

The load balancing policy class to use. If it is not qualified, the driver assumes that it resides in the package `com.khulnasoft.oss.driver.internal.core.loadbalancing`.

KSBulk uses a special policy that infers the local datacenter from the contact points. You can also specify a custom class that implements `LoadBalancingPolicy` and has a public constructor with two arguments: the `DriverContext` and a `String` representing the profile name.

Default: **"com.khulnasoft.oss.driver.internal.core.loadbalancing.DcInferringLoadBalancingPolicy"**.
...
#### --driver.basic.load-balancing-policy.evaluator.class<br />--khulnasoft-java-driver.basic.load-balancing-policy.evaluator.class _&lt;string&gt;_
...
By default, KSBulk ships with a node filter implementation that honors the following settings:
- `khulnasoft-java-driver.basic.load-balancing-policy.evaluator.allow`: a list of host names or host addresses that should be allowed.
- `khulnasoft-java-driver.basic.load-balancing-policy.evaluator.deny`: a list of host names or host addresses that should be denied.

See the description of the above settings for more details.

Default: **"com.khulnasoft.oss.ksbulk.workflow.commons.policies.lbp.SimpleNodeDistanceEvaluator"**.

#### -allow,<br />--driver.basic.load-balancing-policy.evaluator.allow<br />--khulnasoft-java-driver.basic.load-balancing-policy.evaluator.allow _&lt;list&lt;string&gt;&gt;_

An optional list of host names or host addresses that should be allowed to connect. See `khulnasoft-java-driver.basic.contact-points` for a full description of accepted formats.

This option only has effect when the setting `khulnasoft-java-driver.basic.load-balancing-policy.evaluator.class` refers to KSBulk's default node filter implementation: `com.khulnasoft.oss.ksbulk.workflow.commons.policies.lbp.SimpleNodeDistanceEvaluator`.

Note: this option is not compatible with KhulnaSoft Astra databases.

Default: **[]**.

#### -deny,<br />--driver.basic.load-balancing-policy.evaluator.deny<br />--khulnasoft-java-driver.basic.load-balancing-policy.evaluator.deny _&lt;list&lt;string&gt;&gt;_

An optional list of host names or host addresses that should be denied to connect. See `khulnasoft-java-driver.basic.contact-points` for a full description of accepted formats.
...
#### --driver.advanced.protocol.version<br />--khulnasoft-java-driver.advanced.protocol.version _&lt;string&gt;_
...
#### --driver.advanced.protocol.compression<br />--khulnasoft-java-driver.advanced.protocol.compression _&lt;string&gt;_
...
#### --driver.advanced.connection.connect-timeout<br />--khulnasoft-java-driver.advanced.connection.connect-timeout _&lt;string&gt;_
...
#### --driver.advanced.connection.init-query-timeout<br />--khulnasoft-java-driver.advanced.connection.init-query-timeout _&lt;string&gt;_
...
#### --driver.advanced.connection.pool.local.size<br />--khulnasoft-java-driver.advanced.connection.pool.local.size _&lt;number&gt;_

The number of connections in the pool for nodes considered as local.

Default: **8**.

#### --driver.advanced.connection.pool.remote.size<br />--khulnasoft-java-driver.advanced.connection.pool.remote.size _&lt;number&gt;_

The number of connections in the pool for nodes considered as remote. Note that the default load balancing policy used by KSBulk never considers remote nodes, so this setting has no effect when using the default load balancing policy.

Default: **8**.

#### --driver.advanced.connection.max-requests-per-connection<br />--khulnasoft-java-driver.advanced.connection.max-requests-per-connection _&lt;number&gt;_

The maximum number of requests that can be executed concurrently on a connection. This must be between 1 and 32768.

Default: **32768**.

#### --driver.advanced.auth-provider.class<br />--khulnasoft-java-driver.advanced.auth-provider.class _&lt;arg&gt;_
...
- `com.khulnasoft.oss.driver.internal.core.auth`
- `com.khulnasoft.dse.driver.internal.core.auth`
...
#### --driver.advanced.auth-provider.authorization-id<br />--khulnasoft-java-driver.advanced.auth-provider.authorization-id _&lt;string&gt;_
...
#### --driver.advanced.ssl-engine-factory.class<br />--khulnasoft-java-driver.advanced.ssl-engine-factory.class _&lt;string&gt;_

The class of the SSL engine factory. If it is not qualified, the driver assumes that it resides in the package `com.khulnasoft.oss.driver.internal.core.ssl`. The driver provides a single implementation out of the box: `DefaultSslEngineFactory`, that uses the JDK's built-in SSL implementation.
...
#### --driver.advanced.ssl-engine-factory.cipher-suites<br />--khulnasoft-java-driver.advanced.ssl-engine-factory.cipher-suites _&lt;list&lt;string&gt;&gt;_
...
#### --driver.advanced.ssl-engine-factory.hostname-validation<br />--khulnasoft-java-driver.advanced.ssl-engine-factory.hostname-validation _&lt;boolean&gt;_
...
#### --driver.advanced.ssl-engine-factory.truststore-path<br />--khulnasoft-java-driver.advanced.ssl-engine-factory.truststore-path _&lt;string&gt;_
...
#### --driver.advanced.ssl-engine-factory.truststore-password<br />--khulnasoft-java-driver.advanced.ssl-engine-factory.truststore-password _&lt;string&gt;_

The password used to access truststore contents. This setting is only required when using the default SSL factory.

Default: **null**.

#### --driver.advanced.ssl-engine-factory.keystore-path<br />--khulnasoft-java-driver.advanced.ssl-engine-factory.keystore-path _&lt;string&gt;_
...
#### --driver.advanced.ssl-engine-factory.keystore-password<br />--khulnasoft-java-driver.advanced.ssl-engine-factory.keystore-password _&lt;string&gt;_

The password used to access keystore contents. This setting is only required when using the default SSL factory.

Default: **null**.

#### --driver.advanced.retry-policy.class<br />--khulnasoft-java-driver.advanced.retry-policy.class _&lt;string&gt;_

The class of the retry policy. If it is not qualified, the driver assumes that it resides in the package `com.khulnasoft.oss.driver.internal.core.retry`. KSBulk uses by default a special retry policy that opinionately retries most errors up to `max-retries` times.
...
Default: **"com.khulnasoft.oss.ksbulk.workflow.commons.policies.retry.MultipleRetryPolicy"**.

#### -maxRetries,<br />--driver.advanced.retry-policy.max-retries<br />--khulnasoft-java-driver.advanced.retry-policy.max-retries _&lt;number&gt;_

How many times to retry a failed query. Only valid for use with KSBulk's default retry policy (`MultipleRetryPolicy`).

Default: **10**.

#### --driver.advanced.resolve-contact-points<br />--khulnasoft-java-driver.advanced.resolve-contact-points _&lt;boolean&gt;_
...
#### --driver.advanced.address-translator.class<br />--khulnasoft-java-driver.advanced.address-translator.class _&lt;string&gt;_

The class of the translator. If it is not qualified, the driver assumes that it resides in the package `com.khulnasoft.oss.driver.internal.core.addresstranslation`.
...
#### --driver.advanced.timestamp-generator.class<br />--khulnasoft-java-driver.advanced.timestamp-generator.class _&lt;string&gt;_

The class of the microsecond timestamp generator. If it is not qualified, the driver assumes that it resides in the package `com.khulnasoft.oss.driver.internal.core.time`.
...
#### --driver.advanced.continuous-paging.page-size<br />--khulnasoft-java-driver.advanced.continuous-paging.page-size _&lt;number&gt;_
...
#### --driver.advanced.continuous-paging.page-size-in-bytes<br />--khulnasoft-java-driver.advanced.continuous-paging.page-size-in-bytes _&lt;boolean&gt;_
...
#### --driver.advanced.continuous-paging.max-pages<br />--khulnasoft-java-driver.advanced.continuous-paging.max-pages _&lt;number&gt;_

The maximum number of pages to return. The default is zero, which means retrieve all pages.

Default: **0**.

#### --driver.advanced.continuous-paging.max-pages-per-second<br />--khulnasoft-java-driver.advanced.continuous-paging.max-pages-per-second _&lt;number&gt;_

Returns the maximum number of pages per second. The default is zero, which means no limit.

Default: **0**.

#### --driver.advanced.continuous-paging.max-enqueued-pages<br />--khulnasoft-java-driver.advanced.continuous-paging.max-enqueued-pages _&lt;number&gt;_

The maximum number of pages that can be stored in the local queue. This value must be positive. The default is 4.

Default: **4**.

#### --driver.advanced.continuous-paging.timeout.first-page<br />--khulnasoft-java-driver.advanced.continuous-paging.timeout.first-page _&lt;string&gt;_

How long to wait for the coordinator to send the first page.

Default: **"5 minutes"**.

#### --driver.advanced.continuous-paging.timeout.other-pages<br />--khulnasoft-java-driver.advanced.continuous-paging.timeout.other-pages _&lt;string&gt;_

How long to wait for the coordinator to send subsequent pages.

Default: **"5 minutes"**.

#### --driver.advanced.heartbeat.interval<br />--khulnasoft-java-driver.advanced.heartbeat.interval _&lt;string&gt;_
...
#### --driver.advanced.heartbeat.timeout<br />--khulnasoft-java-driver.advanced.heartbeat.timeout _&lt;string&gt;_

How long the driver waits for the response to a heartbeat. If this timeout fires, the heartbeat is considered failed.

Default: **"1 minute"**.

#### --driver.advanced.metrics.session.enabled<br />--khulnasoft-java-driver.advanced.metrics.session.enabled _&lt;list&gt;_
...
#### --driver.advanced.metrics.node.enabled<br />--khulnasoft-java-driver.advanced.metrics.node.enabled _&lt;list&gt;_
