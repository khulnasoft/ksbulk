...
import static com.khulnasoft.oss.ksbulk.runner.ExitStatus.STATUS_ABORTED_TOO_MANY_ERRORS;
import static com.khulnasoft.oss.ksbulk.runner.ExitStatus.STATUS_COMPLETED_WITH_ERRORS;
import static com.khulnasoft.oss.ksbulk.runner.ExitStatus.STATUS_OK;
import static com.khulnasoft.oss.ksbulk.runner.tests.EndToEndUtils.INSERT_INTO_IP_BY_COUNTRY;
import static com.khulnasoft.oss.ksbulk.runner.tests.EndToEndUtils.IP_BY_COUNTRY_MAPPING_CASE_SENSITIVE;
...
import static com.khulnasoft.oss.ksbulk.runner.tests.EndToEndUtils.createIpByCountryCaseSensitiveTable;
import static com.khulnasoft.oss.ksbulk.runner.tests.EndToEndUtils.createIpByCountryTable;
import static com.khulnasoft.oss.ksbulk.runner.tests.EndToEndUtils.createWithSpacesTable;
import static com.khulnasoft.oss.ksbulk.runner.tests.EndToEndUtils.getCheckpointManager;
import static com.khulnasoft.oss.ksbulk.runner.tests.EndToEndUtils.validateCheckpointFile;
import static com.khulnasoft.oss.ksbulk.runner.tests.EndToEndUtils.validateExceptionsLog;
import static com.khulnasoft.oss.ksbulk.runner.tests.EndToEndUtils.validateNumberOfBadRecords;
...
import com.khulnasoft.oss.driver.shaded.guava.common.base.Strings;
...
import com.khulnasoft.oss.ksbulk.io.CompressedIOUtils;
...
import com.khulnasoft.oss.ksbulk.tests.utils.CQLUtils;
import com.khulnasoft.oss.ksbulk.tests.utils.FileUtils;
import com.khulnasoft.oss.ksbulk.workflow.api.log.OperationDirectory;
import com.khulnasoft.oss.ksbulk.workflow.commons.log.checkpoint.Checkpoint;
import com.khulnasoft.oss.ksbulk.workflow.commons.log.checkpoint.CheckpointManager;
import com.khulnasoft.oss.ksbulk.workflow.commons.log.checkpoint.ReplayStrategy;
...
class CSVConnectorEndToEndCCMIT extends EndToEndCCMITBase {
  ...
  void load_from_stdin_and_unload() throws Exception {
    ...
    try {
      ...
      validateCheckpointFile(new URL("std:/"), 500);
      FileUtils.deleteDirectory(logDir);
  ...
  void full_load_unload_compressed_file() throws Exception {
    ...
    assertStatus(status, STATUS_OK);
    validateResultSetSize(24, "SELECT * FROM ip_by_country");
    validateCheckpointFile(resource, 24);
    FileUtils.deleteDirectory(logDir);
  ...
  void full_load_unload_complex() throws Exception {
    ...
    ExitStatus status = new KhulnaSoftBulkLoader(addCommonSettings(args)).run();
    assertStatus(status, STATUS_OK);
    validateCheckpointFile(resource, 2);
    ...
    status = new KhulnaSoftBulkLoader(addCommonSettings(args)).run();
    assertStatus(status, STATUS_OK);
    validateCheckpointFile(unloadDir.resolve("output-000001.csv"), 2);
    assertComplexRows(session);
  }
  ...
  void full_load_unload_counters() throws Exception {
    ...
    ExitStatus status = new KhulnaSoftBulkLoader(addCommonSettings(args)).run();
    assertStatus(status, STATUS_OK);
    validateCheckpointFile(resource, 1);
  ...
  void full_load_unload_large_batches() throws Exception {
    ...
    assertStatus(status, STATUS_OK);
    validateResultSetSize(500, "SELECT * FROM ip_by_country");
    validateCheckpointFile(CsvUtils.CSV_RECORDS, 500);
    FileUtils.deleteDirectory(logDir);
  ...
  void full_load_multiple_files_with_errors() throws Exception {
    ...
    Checkpoint checkpoint1 = manager.getCheckpoint(file1);
    assertThat(checkpoint1.getConsumedSuccessful().sum()).isEqualTo(98);
    assertThat(checkpoint1.getConsumedFailed().sum()).isEqualTo(2);

    Checkpoint checkpoint2 = manager.getCheckpoint(file2);
    assertThat(checkpoint2.getConsumedSuccessful().sum()).isEqualTo(98);
    assertThat(checkpoint2.getConsumedFailed().sum()).isEqualTo(2);

    Checkpoint checkpoint3 = manager.getCheckpoint(file3);
    assertThat(checkpoint3.getConsumedSuccessful().sum()).isEqualTo(98);
    assertThat(checkpoint3.getConsumedFailed().sum()).isEqualTo(2);
  }
  ...
  void full_load_unload_with_spaces() throws Exception {
    ...
    validateCheckpointFile(CsvUtils.CSV_RECORDS_WITH_SPACES, 1);
    FileUtils.deleteDirectory(logDir);
  ...
  void skip_test_load_unload() throws Exception {
    ...
    validateCheckpointFile(CsvUtils.CSV_RECORDS_SKIP.toURI(), 4, 27);
    FileUtils.deleteDirectory(logDir);
  ...
  void error_load_primary_key_cannot_be_null_case_sensitive() throws Exception {
    ...
    validateCheckpointFile(resource.toURI(), 15, 24, false, ReplayStrategy.resume);
  ...
  void should_truncate_and_round() throws Exception {
    ...
    ExitStatus loadStatus = new KhulnaSoftBulkLoader(addCommonSettings(args)).run();
    ...
    ExitStatus unloadStatus = new KhulnaSoftBulkLoader(addCommonSettings(args)).run();
    ...
    loadStatus = new KhulnaSoftBulkLoader(addCommonSettings(args)).run();
    assertStatus(loadStatus, STATUS_OK);
  ...
  void static_columns_full_round_trip(String insertRegular, String insertStatic) throws Exception {
    ...
    // In such cases KSBulk should reject the line '3,null,null' as it has a null clustering column
  ...
  @Test
  void unload_non_parallelizable_custom_where_clause() throws IOException {

    session.execute("DROP TABLE IF EXISTS unload_non_parallelizable");
    session.execute(
        "CREATE TABLE unload_non_parallelizable (pk int, cc int, v int, PRIMARY KEY (pk, cc))");
    session.execute("INSERT INTO unload_non_parallelizable (pk, cc, v) values (0, 1, 2)");
    ...
    List<String> args =
        Lists.newArrayList(
            ...
            session.getKeyspace().get().asInternal(),
            "--schema.query",
            quoteJson("SELECT pk, cc, v FROM unload_non_parallelizable WHERE pk = 0"));
    ...
    validateCheckpointFile(
        URI.create(
            "cql://"
                + session.getKeyspace().get().asInternal()
                + "/unload_non_parallelizable?start=-9223372036854775808&end=-9223372036854775808"),
        1);
  }

  @Test
  void unload_non_parallelizable_limit() throws IOException {
    ...
    List<String> args =
        Lists.newArrayList(
            ...
            session.getKeyspace().get().asInternal(),
            "--schema.query",
            quoteJson("SELECT pk, cc, v FROM unload_non_parallelizable LIMIT 1"));
   ...
   *
   * <pre>{@code
   * ByteBuffer foo = com.khulnasoft.driver.core.TestUtils.serializeForDynamicCompositeType("foo",32);
   * String blobHex = com.khulnasoft.driver.core.utils.Bytes.toHexString(foo.array());
   * }</pre>
   *
  ...
  @Test
  void cas_load_with_errors() throws IOException {
    ...
    Record record1Failed =
        RecordUtils.mappedCSV(
            RecordUtils.DEFAULT_RESOURCE, 1, "pk", "1", "cc", "1", "v", "1"); // will fail
    Record record2Failed =
        RecordUtils.mappedCSV(
            RecordUtils.DEFAULT_RESOURCE, 2, "pk", "1", "cc", "2", "v", "2"); // will fail
    Record record3NotApplied =
        RecordUtils.mappedCSV(
            RecordUtils.DEFAULT_RESOURCE, 3, "pk", "1", "cc", "4", "v", "4"); // will not be applied
