...
def initializeEnvironment() {
  ...
  env.BLUE_OCEAN_URL = "${JENKINS_URL}/blue/organizations/jenkins/tools%2Fksbulk/detail/${BRANCH_NAME}/${BUILD_NUMBER}"
  ...
  sh label: 'Download Apache Cassandra(R) or KhulnaSoft Enterprise', script: '''#!/bin/bash -le
    . ${JABBA_SHELL}
...
def buildAndExecuteTests() {
  sh label: 'Build and execute tests', script: '''#!/bin/bash -le
    ...
    mvn verify $mavenArgs -B \
      -Dksbulk.ccm.CCM_VERSION=${CCM_VERSION} \
      -Dksbulk.ccm.CCM_IS_DSE=${CCM_IS_DSE} \
      -Dksbulk.ccm.JAVA_HOME=${CCM_JAVA_HOME} \
      -Dksbulk.ccm.PATH=${CCM_JAVA_HOME}/bin \
      -Dksbulk.cloud.PROXY_PATH=${HOME}/proxy \
...
def recordArtifacts() {
  if (params.GENERATE_DISTRO && env.CASSANDRA_VERSION.startsWith("3.11")) {
    archiveArtifacts artifacts: 'distribution/target/ksbulk-*.tar.gz', fingerprint: true
    archiveArtifacts artifacts: 'distribution/target/ksbulk-*.zip', fingerprint: true
    archiveArtifacts artifacts: 'distribution/target/ksbulk-*.jar', fingerprint: true
  }
}
