...
import com.khulnasoft.oss.ksbulk.codecs.api.format.geo.JsonGeoFormat;
import com.khulnasoft.oss.ksbulk.codecs.api.format.geo.WellKnownBinaryGeoFormat;
import com.khulnasoft.oss.ksbulk.codecs.api.format.geo.WellKnownTextGeoFormat;
...
import com.khulnasoft.oss.ksbulk.codecs.text.json.JsonCodecUtils;
...
public class CodecSettings {
    ...
    try {
      ...
      try {
        ...
        throw new IllegalArgumentException(
            String.format(
                "Invalid value for ksbulk.codec.%s, expecting temporal in ISO_ZONED_DATE_TIME format, got '%s'",
      ...
      throw ConfigUtils.convertConfigException(e, "ksbulk.codec");
    }
  }
  ...
  private List<BigDecimal> getBooleanNumbers(List<String> booleanNumbersStr) {
    if (booleanNumbersStr.size() != 2) {
      throw new IllegalArgumentException(
          String.format(
              "Invalid value for ksbulk.codec.%s, expecting list with two numbers, got '%s'",
  ...
  private BinaryFormat getBinaryFormat() {
    ...
    switch (binaryFormatStr.toLowerCase()) {
      ...
      default:
        throw new IllegalArgumentException(
            "Invalid value for ksbulk.codec.binary, expecting HEX or BASE64, got "
                + binaryFormatStr);
    }
  }
  ...
  private GeoFormat getGeoFormat() {
    ...
    throw new IllegalArgumentException(
        "Invalid value for ksbulk.codec.geo, expecting WKT, WKB or JSON, got " + geoFormatStr);
  }
        ...
        .forEach(
            tokenizer -> {
              if (tokenizer.countTokens() != 2) {
                throw new IllegalArgumentException(
                    String.format(
                        "Invalid value for ksbulk.codec.booleanStrings, "
