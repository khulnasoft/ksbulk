 ...
 */
package com.khulnasoft.oss.ksbulk.config.shortcuts;

import com.khulnasoft.oss.driver.shaded.guava.common.collect.BiMap;
import com.khulnasoft.oss.driver.shaded.guava.common.collect.ImmutableBiMap;
...
public class ShortcutsFactory {
  ...
  public static BiMap<String, String> createShortcutsMap() {
    return createShortcutsMap(ConfigUtils.standaloneKSBulkReference(), null);
  }
  ...
  private static void addGlobalShortcuts(
    ...
    // global shortcuts
    ConfigObject globalShortcutsConfig = referenceConfig.getObject("ksbulk.metaSettings.shortcuts");
    for (Map.Entry<String, ConfigValue> shortcutEntry : globalShortcutsConfig.entrySet()) {
      String shortcut = shortcutEntry.getKey();
      // global long options are expected to be qualified (i.e. they should start with "ksbulk." or
  ...
  private static void addConnectorShortcuts(
    ...
    String connectorShortcutsPath =
        String.format("ksbulk.connector.%s.metaSettings.shortcuts", connectorName);
    if (referenceConfig.hasPath(connectorShortcutsPath)) {
      ...
      for (Map.Entry<String, ConfigValue> shortcutEntry : connectorShortcutsConfig.entrySet()) {
        ...
        String target =
            "ksbulk.connector." + connectorName + '.' + shortcutEntry.getValue().unwrapped();
