 ...
 *
 * <p>This interface and the URL stream handler discovery mechanism used in KSBulk are very close to
 * the mechanism implemented by default in the JDK starting with JDK 9, see {@code
 * java.net.spi.URLStreamHandlerProvider}. However KSBulk still supports Java 8 and thus has to
...
public interface URLStreamHandlerProvider {
   ...
   * @param config The KSBulk config.
