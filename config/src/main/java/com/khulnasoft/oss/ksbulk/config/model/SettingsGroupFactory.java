 ...
 */
package com.khulnasoft.oss.ksbulk.config.model;
...
public class SettingsGroupFactory {
  ...
  private static final SettingsComparator POPULAR_SECTIONS_FIRST =
      new SettingsComparator("Common", "ksbulk.connector", "ksbulk.schema");
  ...
  /**
   * Creates the configuration sections for KSBulk. The sections created here are used by the help
   * emitter in ksbulk-runner module, and by documentation generation tools found in ksbulk-docs
   * module.
   *
   * @param includeDriver Whether to include a driver section or not.
   */
  public static Map<String, SettingsGroup> createKSBulkConfigurationGroups(boolean includeDriver) {
    ...
    // Now add groups for every top-level setting section in KSBulk config
    ConfigObject ksbulkRoot = referenceConfig.getConfig("ksbulk").root();
    addKsbulkSections(groups, ksbulkRoot, comparator);
    // Now add settings to KSBulk groups
    populateKsbulkSettings(groups, ksbulkRoot, "ksbulk");

    if (includeDriver) {
      Config driverConfig =
          ConfigUtils.standaloneDriverReference().getConfig("khulnasoft-java-driver");
      SettingsGroup driverGroup = new OrderedSettingsGroup();
      populateDriverGroup(driverGroup, driverConfig.root(), "khulnasoft-java-driver");
      groups.put("khulnasoft-java-driver", driverGroup);
    }
    return groups;
  }
  ...
  private static List<String> parseCommonSettings(@NonNull Config referenceConfig) {
    ...
    commonSettings.add("ksbulk.connector.name");
    visitConnectors(
        referenceConfig,
        (connectorName, connectorConfig) -> {
          if (connectorConfig.hasPath("metaSettings.docHints.commonSettings")) {
            connectorConfig.getStringList("metaSettings.docHints.commonSettings").stream()
                ...
                .map(s -> String.format("ksbulk.connector.%s.%s", connectorName, s))
                .forEach(commonSettings::add);
          }
        });
    // KSBulk general common settings are already fully-qualified
    commonSettings.addAll(
        referenceConfig.getStringList("ksbulk.metaSettings.docHints.commonSettings"));
  ...
  private static List<String> parsePreferredSettings(
    ...
    preferredSettings.addAll(
        referenceConfig.getStringList("ksbulk.metaSettings.docHints.preferredSettings"));
  ...
  private static void addKsbulkSections(
    ...
    for (Map.Entry<String, ConfigValue> entry : root.entrySet()) {
      ...
      switch (key) {
        ...
        case "connector":
          ...
          groups.put(
              "ksbulk." + key, new ContainerSettingsGroup("ksbulk." + key, false, comparator));
          for (Map.Entry<String, ConfigValue> child :
              ((ConfigObject) entry.getValue()).entrySet()) {
            if (!ConfigUtils.isLeaf(child.getValue())) {
              groups.put(
                  "ksbulk." + key + "." + child.getKey(),
                  new ContainerSettingsGroup(
                      "ksbulk." + key + "." + child.getKey(), true, comparator));
            }
          }
          break;
        default:
          groups.put(
              "ksbulk." + key, new ContainerSettingsGroup("ksbulk." + key, true, comparator));
          break;
      }
    }
  }

  private static void populateKsbulkSettings(
      @NonNull Map<String, SettingsGroup> groups,
      @NonNull ConfigObject root,
    ...
    for (Map.Entry<String, ConfigValue> entry : root.entrySet()) {
      ...
      if (ConfigUtils.isLeaf(value)) {
        ...
        populateKsbulkSettings(groups, (ConfigObject) value, fullKey);
      }
    }
  }
  ...
  private static void visitConnectors(Config referenceConfig, BiConsumer<String, Config> action) {
    ...
    for (Map.Entry<String, ConfigValue> nonLeafEntry :
        referenceConfig.getObject("ksbulk.connector").entrySet()) {
      connectorChildren.add(nonLeafEntry.getKey());
    }
    for (String connectorName : connectorChildren) {
      ConfigValue child =
          ConfigUtils.getNullSafeValue(referenceConfig, "ksbulk.connector." + connectorName);
