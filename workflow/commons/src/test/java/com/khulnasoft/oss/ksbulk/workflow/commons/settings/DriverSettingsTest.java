...
class DriverSettingsTest {
  ...
  void should_crate_valid_contact_points(String host, int port, String expected)
    ...
    Config oldConfig = TestConfigUtils.createTestConfig("ksbulk.driver");
    Config cpConfig = TestConfigUtils.createTestConfig("ksbulk.executor.continuousPaging");
    Config newConfig =
  ...
  void should_crate_valid_contact_points_with_deprecated_settings(
    ...
    Config oldConfig =
        TestConfigUtils.createTestConfig(
            "ksbulk.driver", "hosts", "[" + StringUtils.quoteJson(host) + "]", "port", port);
    Config cpConfig = TestConfigUtils.createTestConfig("ksbulk.executor.continuousPaging");
    Config newConfig = TestConfigUtils.createTestConfig("khulnasoft-java-driver");
    DriverSettings settings = new DriverSettings(oldConfig, cpConfig, newConfig);
    settings.init(true);
    assertThat(settings.getDriverConfig().getStringList("basic.contact-points"))
        .containsExactly(expected);
  ...
  void should_throw_exception_when_port_not_a_number() {
    Config oldConfig = TestConfigUtils.createTestConfig("ksbulk.driver", "port", "NotANumber");
    Config cpConfig = TestConfigUtils.createTestConfig("ksbulk.executor.continuousPaging");
    Config newConfig = TestConfigUtils.createTestConfig("khulnasoft-java-driver");
    DriverSettings settings = new DriverSettings(oldConfig, cpConfig, newConfig);
    assertThatThrownBy(() -> settings.init(true))
        .isInstanceOf(IllegalArgumentException.class)
        .hasMessageContaining("Invalid value for ksbulk.driver.port, expecting NUMBER, got STRING");
  }
  ...
  void should_throw_exception_when_local_connections_not_a_number() {
    Config oldConfig =
        TestConfigUtils.createTestConfig(
            "ksbulk.driver", "pooling.local.connections", "NotANumber");
    Config cpConfig = TestConfigUtils.createTestConfig("ksbulk.executor.continuousPaging");
    Config newConfig = TestConfigUtils.createTestConfig("khulnasoft-java-driver");
    DriverSettings settings = new DriverSettings(oldConfig, cpConfig, newConfig);
    assertThatThrownBy(() -> settings.init(true))
        ...
        .hasMessageContaining(
            "Invalid value for ksbulk.driver.pooling.local.connections, expecting NUMBER, got STRING");
  }
  ...
  void should_throw_exception_when_remote_connections_not_a_number() {
    Config oldConfig =
        TestConfigUtils.createTestConfig(
            "ksbulk.driver", "pooling.remote.connections", "NotANumber");
    Config cpConfig = TestConfigUtils.createTestConfig("ksbulk.executor.continuousPaging");
    Config newConfig = TestConfigUtils.createTestConfig("khulnasoft-java-driver");
    DriverSettings settings = new DriverSettings(oldConfig, cpConfig, newConfig);
    assertThatThrownBy(() -> settings.init(true))
        .isInstanceOf(IllegalArgumentException.class)
        .hasMessageContaining(
            "Invalid value for ksbulk.driver.pooling.remote.connections, expecting NUMBER, got STRING");
  }

  @Test
  void should_throw_exception_when_local_requests_not_a_number() {
    Config oldConfig =
        TestConfigUtils.createTestConfig("ksbulk.driver", "pooling.local.requests", "NotANumber");
    Config cpConfig = TestConfigUtils.createTestConfig("ksbulk.executor.continuousPaging");
    Config newConfig = TestConfigUtils.createTestConfig("khulnasoft-java-driver");
    DriverSettings settings = new DriverSettings(oldConfig, cpConfig, newConfig);
    assertThatThrownBy(() -> settings.init(true))
        .isInstanceOf(IllegalArgumentException.class)
        .hasMessageContaining(
            "Invalid value for ksbulk.driver.pooling.local.requests, expecting NUMBER, got STRING");
  }

  @Test
  void should_throw_exception_when_remote_requests_not_a_number() {
    Config oldConfig =
        TestConfigUtils.createTestConfig("ksbulk.driver", "pooling.remote.requests", "NotANumber");
    Config cpConfig = TestConfigUtils.createTestConfig("ksbulk.executor.continuousPaging");
    Config newConfig = TestConfigUtils.createTestConfig("khulnasoft-java-driver");
    DriverSettings settings = new DriverSettings(oldConfig, cpConfig, newConfig);
    assertThatThrownBy(() -> settings.init(true))
        .isInstanceOf(IllegalArgumentException.class)
        .hasMessageContaining(
            "Invalid value for ksbulk.driver.pooling.remote.requests, expecting NUMBER, got STRING");
  }

  @Test
  void should_throw_exception_when_fetch_size_not_a_number() {
    Config oldConfig =
        TestConfigUtils.createTestConfig("ksbulk.driver", "query.fetchSize", "NotANumber");
    Config cpConfig = TestConfigUtils.createTestConfig("ksbulk.executor.continuousPaging");
    Config newConfig = TestConfigUtils.createTestConfig("khulnasoft-java-driver");
    DriverSettings settings = new DriverSettings(oldConfig, cpConfig, newConfig);
    assertThatThrownBy(() -> settings.init(true))
        .isInstanceOf(IllegalArgumentException.class)
        .hasMessageContaining(
            "Invalid value for ksbulk.driver.query.fetchSize, expecting NUMBER, got STRING");
  }
  ...
  void should_throw_exception_when_read_timeout_not_a_duration() {
    Config oldConfig =
        TestConfigUtils.createTestConfig(
            "ksbulk.driver", "socket.readTimeout", "\"I am not a duration\"");
    Config cpConfig = TestConfigUtils.createTestConfig("ksbulk.executor.continuousPaging");
    Config newConfig = TestConfigUtils.createTestConfig("khulnasoft-java-driver");
    DriverSettings settings = new DriverSettings(oldConfig, cpConfig, newConfig);
    assertThatThrownBy(() -> settings.init(true))
        .isInstanceOf(IllegalArgumentException.class)
        .hasMessageContaining("Invalid value for ksbulk.driver.socket.readTimeout")
  ...
  void should_throw_exception_when_max_retries_not_a_number() {
    Config oldConfig =
        TestConfigUtils.createTestConfig("ksbulk.driver", "policy.maxRetries", "NotANumber");
    Config cpConfig = TestConfigUtils.createTestConfig("ksbulk.executor.continuousPaging");
    Config newConfig = TestConfigUtils.createTestConfig("khulnasoft-java-driver");
    DriverSettings settings = new DriverSettings(oldConfig, cpConfig, newConfig);
    assertThatThrownBy(() -> settings.init(true))
        .isInstanceOf(IllegalArgumentException.class)
        .hasMessageContaining(
            "Invalid value for ksbulk.driver.policy.maxRetries, expecting NUMBER, got STRING");
  }

  @Test
  void should_throw_exception_when_idempotence_not_a_boolean() {
    Config oldConfig =
        TestConfigUtils.createTestConfig("ksbulk.driver", "query.idempotence", "NotABoolean");
    Config cpConfig = TestConfigUtils.createTestConfig("ksbulk.executor.continuousPaging");
    Config newConfig = TestConfigUtils.createTestConfig("khulnasoft-java-driver");
    DriverSettings settings = new DriverSettings(oldConfig, cpConfig, newConfig);
    assertThatThrownBy(() -> settings.init(true))
        .isInstanceOf(IllegalArgumentException.class)
        .hasMessageContaining(
            "Invalid value for ksbulk.driver.query.idempotence, expecting BOOLEAN, got STRING");
  }

  @Test
  void should_throw_exception_when_timestamp_generator_invalid() {
    Config oldConfig =
        TestConfigUtils.createTestConfig("ksbulk.driver", "timestampGenerator", "Unknown");
    Config cpConfig = TestConfigUtils.createTestConfig("ksbulk.executor.continuousPaging");
    Config newConfig = TestConfigUtils.createTestConfig("khulnasoft-java-driver");
    DriverSettings settings = new DriverSettings(oldConfig, cpConfig, newConfig);
    assertThatThrownBy(() -> settings.init(true))
  ...
  void should_throw_exception_when_address_translator_invalid() {
    Config oldConfig =
        TestConfigUtils.createTestConfig("ksbulk.driver", "addressTranslator", "Unknown");
    Config cpConfig = TestConfigUtils.createTestConfig("ksbulk.executor.continuousPaging");
    Config newConfig = TestConfigUtils.createTestConfig("khulnasoft-java-driver");
    DriverSettings settings = new DriverSettings(oldConfig, cpConfig, newConfig);
    assertThatThrownBy(() -> settings.init(true))
        .isInstanceOf(IllegalArgumentException.class)
        .hasMessageContaining(
            "Invalid value for ksbulk.driver.protocol.addressTranslator, expecting IdentityTranslator, got 'Unknown'");
  }

  @Test
  void should_throw_exception_when_compression_invalid() {
    Config oldConfig =
        TestConfigUtils.createTestConfig("ksbulk.driver", "protocol.compression", "Unknown");
    Config cpConfig = TestConfigUtils.createTestConfig("ksbulk.executor.continuousPaging");
    Config newConfig = TestConfigUtils.createTestConfig("khulnasoft-java-driver");
    DriverSettings settings = new DriverSettings(oldConfig, cpConfig, newConfig);
    assertThatThrownBy(() -> settings.init(true))
        .isInstanceOf(IllegalArgumentException.class)
        .hasMessageContaining(
            "Invalid value for ksbulk.driver.protocol.compression, expecting one of NONE, SNAPPY, LZ4, got 'Unknown'");
  }

  @Test
  void should_throw_exception_when_consistency_invalid() {
    Config oldConfig =
        TestConfigUtils.createTestConfig("ksbulk.driver", "query.consistency", "Unknown");
    Config cpConfig = TestConfigUtils.createTestConfig("ksbulk.executor.continuousPaging");
    Config newConfig = TestConfigUtils.createTestConfig("khulnasoft-java-driver");
    DriverSettings settings = new DriverSettings(oldConfig, cpConfig, newConfig);
    assertThatThrownBy(() -> settings.init(true))
        .isInstanceOf(IllegalArgumentException.class)
        .hasMessageContaining(
            "Invalid value for ksbulk.driver.query.consistency, expecting one of ANY, ONE, TWO, THREE, QUORUM, ALL, LOCAL_ONE, LOCAL_QUORUM, EACH_QUORUM, SERIAL, LOCAL_SERIAL, got: 'Unknown'");
  }

  @Test
  void should_throw_exception_when_serial_consistency_invalid() {
    Config oldConfig =
        TestConfigUtils.createTestConfig("ksbulk.driver", "query.serialConsistency", "Unknown");
    Config cpConfig = TestConfigUtils.createTestConfig("ksbulk.executor.continuousPaging");
    Config newConfig = TestConfigUtils.createTestConfig("khulnasoft-java-driver");
    DriverSettings settings = new DriverSettings(oldConfig, cpConfig, newConfig);
    assertThatThrownBy(() -> settings.init(true))
        .isInstanceOf(IllegalArgumentException.class)
        .hasMessageContaining(
            "Invalid value for ksbulk.driver.query.serialConsistency, expecting one of ANY, ONE, TWO, THREE, QUORUM, ALL, LOCAL_ONE, LOCAL_QUORUM, EACH_QUORUM, SERIAL, LOCAL_SERIAL, got: 'Unknown'");
  }

  @Test
  void should_throw_exception_when_heartbeat_invalid() {
    Config oldConfig =
        TestConfigUtils.createTestConfig("ksbulk.driver", "pooling.heartbeat", "NotADuration");
    Config cpConfig = TestConfigUtils.createTestConfig("ksbulk.executor.continuousPaging");
    Config newConfig = TestConfigUtils.createTestConfig("khulnasoft-java-driver");
    DriverSettings settings = new DriverSettings(oldConfig, cpConfig, newConfig);
    assertThatThrownBy(() -> settings.init(true))
        .isInstanceOf(IllegalArgumentException.class)
        .hasMessageContaining(
            "Invalid value for ksbulk.driver.pooling.heartbeat: No number in duration value 'NotADuration'");
  }
  ...
  void should_log_warning_when_deprecated_driver_setting_present(
      ...
      throws GeneralSecurityException, IOException {
    Config oldConfig = TestConfigUtils.createTestConfig("ksbulk.driver", deprecatedSetting, value);
    Config cpConfig = TestConfigUtils.createTestConfig("ksbulk.executor.continuousPaging");
    Config newConfig = TestConfigUtils.createTestConfig("khulnasoft-java-driver");
    DriverSettings settings = new DriverSettings(oldConfig, cpConfig, newConfig);
    settings.init(true);
    assertThat(logs)
        .hasMessageContaining(
            String.format(
                "Setting ksbulk.driver.%s is deprecated and will be removed in a future release; "
                    + "please configure the driver directly using --khulnasoft-java-driver.%s instead.",
  ...
  void should_log_warning_when_deprecated_driver_setting_present_with_shortcut(
    ...
    assertThat(logs)
        .hasMessageContaining(
            String.format(
                "Setting ksbulk.driver.%s is deprecated and will be removed in a future release; "
                    + "please configure the driver directly using --khulnasoft-java-driver.%s (or -%s) instead.",
  ...
  void should_log_warning_when_deprecated_continuous_paging_setting_present(
    ...
    Config cpConfig =
        TestConfigUtils.createTestConfig(
            "ksbulk.executor.continuousPaging", deprecatedSetting, value);
    Config newConfig = TestConfigUtils.createTestConfig("khulnasoft-java-driver");
    DriverSettings settings = new DriverSettings(oldConfig, cpConfig, newConfig);
    settings.init(false);
    assertThat(logs)
        .hasMessageContaining(
            String.format(
                "Setting ksbulk.executor.continuousPaging.%s is deprecated and will be removed in a future release; "
                    + "please configure the driver directly using --khulnasoft-java-driver.%s instead.",
                deprecatedSetting, newSetting));
  }
  ...
  void should_log_warning_when_obsolete_LBP_setting_present(
      ...
      @LogCapture(level = WARN, value = DriverSettings.class) LogInterceptor logs)
      throws GeneralSecurityException, IOException {
    Config oldConfig = TestConfigUtils.createTestConfig("ksbulk.driver", obsoleteSetting, value);
    Config cpConfig = TestConfigUtils.createTestConfig("ksbulk.executor.continuousPaging");
    Config newConfig = TestConfigUtils.createTestConfig("khulnasoft-java-driver");
    DriverSettings settings = new DriverSettings(oldConfig, cpConfig, newConfig);
    settings.init(true);
    assertThat(logs)
        .hasMessageContaining(
            String.format(
                "Setting ksbulk.driver.%s has been removed and is not honored anymore; "
                    + "please remove it from your configuration. "
                    + "To configure the load balancing policy, use "
                    + "--khulnasoft-java-driver.basic.load-balancing-policy.* instead",
                obsoleteSetting));
  }
  ...
  void should_throw_exception_when_page_unit_invalid() {
    ...
    Config cpConfig =
        TestConfigUtils.createTestConfig(
            "ksbulk.executor.continuousPaging", "pageUnit", "NotAPageUnit");
    Config newConfig = TestConfigUtils.createTestConfig("khulnasoft-java-driver");
    DriverSettings settings = new DriverSettings(oldConfig, cpConfig, newConfig);
    assertThatThrownBy(() -> settings.init(false))
        .isInstanceOf(IllegalArgumentException.class)
        .hasMessageContaining(
            "Invalid value for ksbulk.executor.continuousPaging.pageUnit, expecting one of BYTES, ROWS, got 'NotAPageUnit'");
  }
  ...
  void should_log_warn_when_cloud_and_explicit_contact_points_legacy(
      ...
      throws GeneralSecurityException, IOException {
    assumeFalse(PlatformUtils.isWindows());
    Config oldConfig = TestConfigUtils.createTestConfig("ksbulk.driver", "hosts", "[host.com]");
    Config cpConfig = TestConfigUtils.createTestConfig("ksbulk.executor.continuousPaging");
    Config newConfig =
        TestConfigUtils.createTestConfig(
            "khulnasoft-java-driver", "basic.cloud.secure-connect-bundle", "/path/to/bundle");
  ...
  void should_log_warning_when_when_cloud_and_SSL_explicitly_set_legacy(
      ...
      throws GeneralSecurityException, IOException {
    assumeFalse(PlatformUtils.isWindows());
    Config oldConfig = TestConfigUtils.createTestConfig("ksbulk.driver", "ssl.provider", "JDK");
    Config cpConfig = TestConfigUtils.createTestConfig("ksbulk.executor.continuousPaging");
    Config newConfig =
        TestConfigUtils.createTestConfig(
            "khulnasoft-java-driver", "basic.cloud.secure-connect-bundle", "/path/to/bundle");
    ...
    assertThat(logs)
        .hasMessageContaining(
            "Setting ksbulk.driver.ssl.* is deprecated and will be removed in a future release; "
                + "please configure the driver directly using "
                + "--khulnasoft-java-driver.advanced.ssl-engine-factory.* instead.")
        .hasMessageContaining(
            "Explicit SSL configuration provided together with a cloud secure connect bundle: "
                + "SSL settings will be ignored.");
    assertThat(settings.getDriverConfig().hasPath("advanced.ssl-engine-factory")).isFalse();
